...
describe('scrollbar geometry', () => {
  ...
  it('can jump to a track position and continue dragging from there', () => {
    const bar = new ScrollBar({} as UiSkin, { trackClick: 'jump' });
    bar.setBounds({ x: 0, y: 0, width: 14, height: 100 });
    bar.setMetrics(20, 5);
    expect(bar.pointerDown({ x: 7, y: 90 })).toBe(true);
    expect(bar.position).toBeGreaterThan(10);
    expect(bar.pointerMove({ x: 7, y: 20 })).toBe(true);
    expect(bar.position).toBeLessThan(5);
    expect(bar.pointerUp()).toBe(true);
  });
