...
export class UiInventoryInteractionModel {
  ...
  private pickupAll(ref: UiInventorySlotRef, button: CursorClickButton): UiInventoryAction {
    ...
    const result = pickupAllToCursor(
      this.containersValue,
      this.cursorValue,
      Object.keys(this.containersValue),
      BOOTSTRAP_ITEM_CONTAINER_CONTENT,
    );
    return this.applyCursorResult(result);
  }
...
export function drawUiInventorySlotBacking(
  ...
  context.save();
  // The complete gray authored state keeps its fill and border at one muted
  // strength; it never receives the generic inset-only disabled fallback.
  if (disabled) context.globalAlpha *= 0.6;
