...
export interface StorageFrameSpec {
  ...
  /** Reserved space between pane content and the hotbar/divider for authored
   * frame buttons. Kept optional so every legacy composition is unchanged. */
...
export function layoutStorageFrame(
  ...
  const footerHeight = Math.max(0, spec.footerHeight ?? 0);
  const minimumHeight = PANE_GRID_TOP + paneHeight + footerHeight + (spec.hotbar === undefined
