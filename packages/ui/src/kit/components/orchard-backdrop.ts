...
export function uiOrchardBackdrop(layout?: UiStyle): UiElement {
  return new UiElement({kind:'orchard-backdrop',animated:true,style:{width:'grow',height:'grow',...layout},paint(element,{context,now,reducedMotion}){context.translate(element.rect.x,element.rect.y);drawOrchardBackdrop(context,element.rect.width,element.rect.height,reducedMotion?0:now);}});
}
