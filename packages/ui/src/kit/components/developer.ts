...
export type UiDeveloperAction = 'previous-day'|'next-day'|'weather'|'wind'|'lighting-effects'|'ore-preview';
...
export function uiDeveloper(options: UiDeveloperOptions): UiDeveloperElement {
  let model=options.model,selected=options.tab??'world';
  ...
  const pages:Record<UiDeveloperTab,UiElement>={
    ...
    render:page([lighting,ore,...['COLLISION BOUNDS','PATHFINDING OVERLAY'].map(label=>uiSwitch({label,value:false,disabled:true,layout:{width:'grow',shrink:0}}))]),
  };
  const tabs=uiTabs({id:'developer.pages',label:'Developer pages',value:selected,tabs:UI_DEVELOPER_TABS.map(id=>({id,label:id.toUpperCase(),content:pages[id]})),onChange:id=>{selected=id as UiDeveloperTab;options.onTab?.(selected);}});
  ...
  const updateDeveloper=(nextModel:UiDeveloperModel)=>{
    model=nextModel;
    for(const control of [previous,next,time,weather,wind,lighting,ore])control.setDisabled(!model.canAdministerWorld);
    ...
    lighting.setProps({value:model.lightingEffectsDisabled!==true},false);ore.setProps({value:model.cellarOrePreview===true},false);
  };
  updateDeveloper(model);
  return Object.defineProperty(Object.assign(frame,{updateDeveloper,selectDeveloperTab(tab:UiDeveloperTab){tabs.selectTab(tab);}}),'selectedTab',{get:()=>selected}) as UiDeveloperElement;
}
