...
export function uiSkills(options:UiSkillsOptions):UiSkillsElement {
 ...
 const updateSkills=(next:SkillTreeModel)=>{const nodesChanged=model.nodes!==next.nodes;model=next;if(nodesChanged){selectTrack(track);}const nextKey=JSON.stringify(next,(_key,value)=>typeof value==='bigint'?value.toString():value);if(key===nextKey)return;key=nextKey;refresh();};
