...
export interface UiGatewayModel {
  ...
  readonly version?: string; readonly allowLocalPreview?: boolean;
}
...
export function uiGateway(options: UiGatewayOptions): UiGatewayElement {
  ...
  const invoke=(action:UiGatewayAction)=>{
    ...
    if(action==='toggle-preview'&&!model.allowLocalPreview)return;
  ...
  const submit=()=>invoke(model.localPreview?'continue-local':model.signedIn?'enter-world':'sign-in');
  ...
  const frame=uiFrame({id:'game.gateway',header:{title:'ORCHARD & CELLAR',content:uiRibbon({label:'ORCHARD & CELLAR',layout:{width:'grow'}})},layout:{width:'grow',height:'grow',...options.layout},children:[uiScrollArea({gap:8},[uiFlex({direction:'row',width:'grow',gap:4,align:'center'},[...(emblem?[emblem]:[]),version]),message,account,local,toggle])]});
  const clearName=()=>{editor.setValue('');frame.invalidate();};
  const selectProfile=(index:number)=>{if(model.busy||!model.localPreview||!model.profiles[index])return;clearName();options.onSelectProfile(index);};
  const updateGateway=(next:UiGatewayModel)=>{
    ...
    for(const [id,button]of controls){button.setDisabled(model.busy);button.setStyle({visible:id==='toggle-preview'?model.allowLocalPreview===true:id==='continue-local'?model.localPreview:['enter-world','sign-out'].includes(id)?model.signedIn:!model.signedIn});}
  ...
  updateGateway(model);
  return Object.assign(frame,{editor,updateGateway,submit,clearName,focusName(){if(model.localPreview&&!model.busy)input.requestFocus();},selectAdjacentProfile(direction:-1|1){if(model.profiles.length)selectProfile((model.selected+direction+model.profiles.length)%model.profiles.length);}});
}
