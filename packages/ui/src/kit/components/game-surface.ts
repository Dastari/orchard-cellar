...
export function uiGameSurface(options: UiGameSurfaceOptions): UiElement {
  ...
  switch (options.id) {
    ...
    case 'skills': return uiSkills({model:options.skills??{tracks:[],ranks:[],balanceBronze:0n},artwork:options.skillArtwork,purchase:id=>invoke('learn',id),reset:track=>invoke('reset',track),onNavigate:page=>invoke('navigate',page),onClose:()=>invoke('close'),layout:options.layout}).setProps({migrationSurface:options.id});
