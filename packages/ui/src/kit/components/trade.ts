...
import { BACKPACK_SLOT_OFFSET,HOTBAR_SLOT_COUNT,itemDefinition,isUniqueQuestItemKind,coinPurseFromBronze,BRONZE_PER_GOLD,BRONZE_PER_SILVER } from '@orchard/sim';
...
export function uiTrade(options:UiTradeOptions):UiTradeElement {
 ...
 const updateTrade=(next:TradeUiModel)=>{
  ...
  if(key!==structure){structure=key;for(const child of [...content.children])child.dispose();
   ...
   for(const [grid,owner]of [[ownGrid,()=>model.identityHex],[otherGrid,otherHex]]as const)for(const [index,cell]of [...grid.children].entries()){grid.remove(cell);grid.append(uiTooltip(()=>{const item=offer(owner(),index);return item?(itemDefinition(item.itemKind)?.displayName??item.itemKind):'';},cell,{width:uiFixed(28),height:uiFixed(31)}));}
   const carried=uiInventoryGrid({container:'trade-inventory',cells:inventory().map(row=>({id:String(row.slot),index:row.slot})),columns:10,slotSize:'sm',gap:2,artwork:options.artwork,allowSecondary:true,stack:index=>model.inventorySlots.find(row=>row.slot===index)??null,onActivate:(index,event)=>{const row=model.inventorySlots.find(row=>row.slot===index),free=Array.from({length:6},(_,i)=>i).find(i=>!offer(model.identityHex,i));if(!row||row.itemKind==='empty'||row.quantity<=0||isUniqueQuestItemKind(row.itemKind)||free===undefined)return;options.callbacks.offerItem(model.session.id,index,free,event.button===2?1:row.quantity);}});
