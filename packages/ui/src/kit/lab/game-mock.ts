...
import { bootstrapContentDefinitions, bootstrapContentRegistry, runtimePlayerAppearanceCatalog, homesteadBuildDefinitions, SKILL_NODE_DEFINITIONS, itemDefinition, ITEM_ECONOMY, merchantOffers, type FrameContentDefinition } from '@orchard/sim';
...
export function uiLabGameOptions(id: UiLabMigrationSurfaceId, props: Readonly<Record<string, unknown>>, mock: UiLabMocks): UiGameSurfaceOptions {
  ...
  if(id==='skills')for(const node of SKILL_NODE_DEFINITIONS)mock.requestAsset?.(node.iconAsset);
