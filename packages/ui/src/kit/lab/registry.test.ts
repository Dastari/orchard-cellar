...
beforeAll(async () => { art = await uiTestArt(); for (const name of ['tile_cf_grass', 'tile_cf_stone_cliff_variants']) actorAssets.set(name, uiTestAsset(name, 'tiles')); const actor = uiTestAsset('npc_cf_desert_person_01', 'characters'); actorAssets.set(actor.name, actor); for (const node of SKILL_NODE_DEFINITIONS) actorAssets.set(`icon_skill_${node.id}`,uiTestAsset(`icon_skill_${node.id}`,'ui')); for (const name of UI_CHARACTER_PORTRAIT_ASSETS) actorAssets.set(name,uiTestAsset(name,'characters')); for (const name of ['icon_cf_quest_offer', 'icon_cf_quest_complete']) actorAssets.set(name, uiTestAsset(name, 'ui')); for (const kind of UI_LAB_ITEM_KINDS) { const name = itemDefinition(kind)?.iconKey; if (name) actorAssets.set(name, uiTestAsset(name, name.startsWith('item_') || name.startsWith('prop_') ? 'props' : 'ui')); } });
