...
describe('semantic canvas button', () => {
  ...
  it('owns the danger surface and a high-contrast white icon treatment', () => {
    expect(iconButtonAppearance('danger')).toEqual({
      asset: 'buttonDeny',
      opacity: 0.95,
      filter: LIGHT_ICON_FILTER,
    });
    expect(iconButtonAppearance('danger', 'disabled')).toEqual({
      asset: 'buttonDeny',
      opacity: 0.35,
      filter: 'none',
    });
