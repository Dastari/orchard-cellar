...
export const LIGHT_ICON_FILTER = 'brightness(0) invert(1)' as const;

export interface DrawIconButtonOptions {
  readonly icon: UiIconAsset;
  readonly tone?: ButtonTone;
  readonly state?: ButtonVisualState;
}
...
export interface IconButtonAppearance {
  readonly asset: 'button' | 'buttonConfirm' | 'buttonDeny';
  readonly opacity: number;
  readonly filter: string;
}

/** Shared semantic styling for full-size icon-only actions. Destructive and
 * confirm actions use a light glyph so their icon remains legible on colour. */
export function iconButtonAppearance(
  tone: ButtonTone = 'neutral',
  state: ButtonVisualState = 'idle',
): IconButtonAppearance {
  return {
    asset: tone === 'success' ? 'buttonConfirm' : tone === 'danger' ? 'buttonDeny' : 'button',
    opacity: state === 'disabled' ? 0.35 : 0.95,
    filter: state !== 'disabled' && tone !== 'neutral' ? LIGHT_ICON_FILTER : 'none',
  };
}

export function drawIconButton(
  context: CanvasRenderingContext2D,
  skin: Pick<UiSkin, 'button' | 'buttonConfirm' | 'buttonDeny'>,
  rect: UiRect,
  options: DrawIconButtonOptions,
): void {
  const state = options.state ?? 'idle';
  const appearance = iconButtonAppearance(options.tone, state);
  drawUiSkinAsset(context, skin[appearance.asset], rect, state);
  drawUiIconAsset(
    context,
    options.icon,
    smallButtonIconRect(rect, state),
    appearance.opacity,
    appearance.filter,
  );
}
