...
export function touchControlLayout(
  ...
  const offset = Math.min(bottomOffset, Math.max(0, Math.min(centerY - 38, buttonCenterY - 56)));
  const mirror = (rect: UiRect): UiRect => ({
    ...rect, x: swapped ? width - rect.x - rect.width : rect.x, y: rect.y - offset,
  });
  return {
    joystickCenter: { x: swapped ? width - JOYSTICK_RADIUS - 12 : JOYSTICK_RADIUS + 12, y: centerY - offset },
...
export class TouchControls {
  ...
  get blockHeld(): boolean { return this.visible && [...this.pressedActions.values()].includes('block'); }
  ...
  pointerDown(
    ...
    const layout = touchControlLayout(width, height, this.preferences);
    ...
    for (const action of ['dodge','block'] as const) {
      if (containsPoint(layout[`${action}Button`], point)) {
        this.pressedActions.set(pointerId, action); this.capturedPointers.add(pointerId); return action;
      }
    }
  ...
  pointerMove(point: UiPoint, pointerId: number, width: number, height: number): boolean {
    ...
    this.updateMovementPoint(point, touchControlLayout(width, height, this.preferences));
    return true;
  }
  ...
  draw(
    ...
    for (const action of ['dodge','block'] as const) {
      const rect=layout[`${action}Button`];
      drawUiSkinAsset(context, skin.button, rect, [...this.pressedActions.values()].includes(action) ? 'pressed' : 'idle');
      drawPixelText(context, ui, action === 'dodge' ? 'DODGE' : 'BLOCK', rect.x+rect.width/2, rect.y+6, {align:'center',color:'#5c3427'});
      drawPixelText(context, ui, action === 'dodge' ? '18 V' : 'HOLD', rect.x+rect.width/2, rect.y+18, {align:'center',color:'#5c3427'});
    }
