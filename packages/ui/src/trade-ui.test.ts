...
import {
  TradeUi,
  tradeItemDisplayName,
  tradeItemIsOfferable,
  type TradeUiCallbacks,
  type TradeUiModel,
} from './trade-ui.js';
...
describe('trade UI', () => {
  ...
  it('uses active authored item policy for arbitrary renamed trade inventory', () => {
    ...
    const ui = new TradeUi({} as UiSkin, {} as PixelUi, {} as never, moneyInputs(), handlers);
    ui.update({
      ...model('active'), contentRegistry,
      inventorySlots: [{ slot: 0, itemKind: 'moon_ledger', quantity: 7 }],
    });
    expect(ui.pointerDown({ x: 135, y: 160 }, 0)).toBe(true);
    expect(handlers.offerItem).not.toHaveBeenCalled();
    ...
    ui.update({
      ...model('active'), contentRegistry,
      inventorySlots: [{ slot: 0, itemKind: 'unknown_ledger', quantity: 1 }],
    });
    expect(ui.pointerDown({ x: 135, y: 160 }, 0)).toBe(true);
    expect(handlers.offerItem).not.toHaveBeenCalled();
  });
