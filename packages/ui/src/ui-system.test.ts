...
describe('retained UI layout', () => {
  ...
  it('lays out rows, columns, flex, anchors, and minimum sizes in whole pixels', () => {
    expect(layoutUiFlex({ x: 0, y: 0, width: 31, height: 10 }, [
      { minSize: { width: 5, height: 4 }, grow: 1 }, { minSize: { width: 5, height: 6 }, grow: 1 },
    ], { direction: 'row', gap: 1, align: 'center' })).toEqual([
      { x: 0, y: 3, width: 15, height: 4 }, { x: 16, y: 2, width: 15, height: 6 },
    ]);
    expect(layoutUiFlex({ x: 2, y: 3, width: 20, height: 11 }, [
      { minSize: { width: 4, height: 3 } }, { minSize: { width: 5, height: 4 } },
    ], { direction: 'column', gap: 2, align: 'end' })).toEqual([
      { x: 18, y: 3, width: 4, height: 3 }, { x: 17, y: 8, width: 5, height: 4 },
    ]);
    expect(layoutUiAnchoredRect({ x: 10, y: 20, width: 100, height: 80 }, { width: 30, height: 20 }, {
      targetAnchor: 'bottom_right', selfAnchor: 'bottom_right',
    }))
...
describe('bindings and routing', () => {
  it('uses reusable typed item slots for equipment acceptance', () => {
    const watch = new ItemSlot('watch', 'equipment', 2, EQUIPMENT_SLOT_RESTRICTIONS[2]);
    expect(watch.accepts('watch')).toBe(true);
    expect(watch.accepts('backpack')).toBe(false);
    watch.enabled = false;
    expect(watch.accepts('watch')).toBe(false);
  });

  it('renders sparse subscribed rows through a self binding', () => {
    expect(bindContainerSlots('self:backpack', 3, [
    ...
    ], { 'self:backpack': 'player:local:backpack' }))
      .toEqual([
        { containerId: 'player:local:backpack', index: 0, itemKind: 'axe', quantity: 1 },
        { containerId: 'player:local:backpack', index: 1, itemKind: 'wood', quantity: 12 },
