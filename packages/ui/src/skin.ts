...
export async function loadGameplayUiSkin(): Promise<GameplayUiSkin> {
  const [generatedSkin, skillIconEntries] = await Promise.all([
    ...
    Promise.all(SKILL_NODE_DEFINITIONS.map(async (node) => [
      node.id,
      await loadGeneratedAsset(`icon_skill_${node.id}`, 'summer'),
    ] as const)),
  ]);
  return {
    ...generatedSkin,
