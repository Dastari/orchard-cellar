...
export interface UiSkin {
  ...
  readonly moonPhase: LoadedAsset;
...
/** One in-flight/settled request per authored atlas key. Failed requests are
 * evicted so a later manifest/network retry is not permanently poisoned. */
...
/** Resolves only nodes in active authored trees. Unknown and retired content
 * deliberately produces no request; callers can render their existing
 * missing-art treatment without resurrecting bootstrap identity policy. */
