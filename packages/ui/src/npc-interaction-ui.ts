...
export class NpcInteractionUi {
  ...
  private drawShop(context: CanvasRenderingContext2D, layout: InteractionLayout): void {
    ...
    if (this.hoveredItemKind) {
      ...
      const economy = authoredItem?.economy
      ...
      if (economy) {
        ...
        drawUiLabelPlate(context, this.skin, tooltip);
        drawPixelText(context, this.fonts, (authoredItem?.displayName ?? itemDefinition(this.hoveredItemKind)?.displayName ?? this.hoveredItemKind).toUpperCase(), tooltip.x + tooltip.width / 2, tooltip.y + 4, { align: 'center', color: '#51351f' });
      }
    }
  }
}
