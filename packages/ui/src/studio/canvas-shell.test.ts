...
describe('Studio canvas shell UI-Lab primitive adapter', () => {
  ...
  it('loads the production bitmap fonts and UI skin together', async () => {
    ...
    expect(render.loadSkin.mock.calls[0]?.[0]).toHaveLength(16);
  ...
  it('routes every shell node through the shared canvas primitives', () => {
    ...
    expect(render.button).toHaveBeenCalledTimes(4);
    expect(render.ribbon).toHaveBeenCalledWith(expect.anything(), 'TOOLS',
    ...
    expect(render.text).toHaveBeenCalledTimes(7);
  ...
  it('draws topmost overlay nodes without repainting the shell backdrop', () => {
    ...
    expect(render.frame).toHaveBeenCalledOnce();
    expect(render.frame.mock.calls[0]?.[3]).toBe('thin');
  });
});
