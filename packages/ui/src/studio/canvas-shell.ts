...
export const STUDIO_CANVAS_UI_ICONS = Object.freeze([
  ...
  for (const node of nodes) {
    ...
    switch (node.kind) {
      ...
      case 'alpha_grid':
        ...
        break;
      case 'ribbon':
        studioRibbon(art).drawSingle(context, node.label ?? '', node.bounds);
        break;
      case 'button':
      case 'tab':
      ...
      case 'tooltip':
        ...
        drawUiFrame(context, art.skin, node.bounds, 'thin');
        drawText(context, art, { ...node, textScale: node.textScale ?? 1 });
        break;
    }
    context.restore();
  }
