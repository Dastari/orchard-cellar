import { rogueUpgradeDefinition, type RogueRarity } from '@orchard/sim';
...
function magnitudeLabel(offer: RogueUiOffer): string {
  const definition = rogueUpgradeDefinition(offer.upgradeId);
  if (definition === undefined) return `+${Math.round(offer.magnitudePermille / 10)}%`;
...
export function drawRogueRewardOverlay(
  ...
  offers.forEach((offer, index) => {
    ...
    drawPixelText(context, ui, magnitudeLabel(offer), rect.x + 7, rect.y + 32, {
