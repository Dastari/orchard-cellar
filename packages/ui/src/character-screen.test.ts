...
import { bootstrapContentRegistry, runtimePlayerAppearanceCatalog } from '@orchard/sim';
import {
...
const appearanceCatalog = runtimePlayerAppearanceCatalog(bootstrapContentRegistry())!;
...
describe('character screen logic', () => {
  ...
  it('cycles every authored appearance axis in both directions with wraparound', () => {
    expect(cycleAppearanceValue(appearance, appearanceCatalog, 'hairKind', 1).hairKind).toBe('hair_2_black');
    expect(cycleAppearanceValue(appearance, appearanceCatalog, 'hairKind', -1).hairKind).toBe('hair_6_brown');
    expect(cycleAppearanceValue(appearance, appearanceCatalog, 'shirtKind', -1).shirtKind).toBe('farmer_white_brown');
    expect(cycleAppearanceValue(appearance, appearanceCatalog, 'pantsKind', 1).pantsKind).toBe('farmer_black');
    expect(cycleAppearanceValue(appearance, appearanceCatalog, 'shoesKind', -1).shoesKind).toBe('red');
  });
  ...
  it('shows all ten equipment positions while keeping turn controls clear of their label', () => {
    expect(CHARACTER_EQUIPMENT_PRESENTATION).toHaveLength(10);
    expect(new Set(CHARACTER_EQUIPMENT_PRESENTATION.map(({ slotIndex }) => slotIndex)).size).toBe(10);
