...
it('does not claim a second thumb while the first owns movement', () => {
  const controls = new TouchControls(true), layout = touchControlLayout(390,844);
  expect(controls.pointerDown(layout.joystickCenter,1,'touch',390,844)).toBe('movement');
  expect(controls.pointerDown(layout.joystickCenter,2,'touch',390,844)).toBeNull();
  expect(controls.ownsPointer(2)).toBe(false);
  controls.pointerUp(1);
  expect(controls.pointerDown(layout.joystickCenter,2,'touch',390,844)).toBe('movement'); controls.dispose();
});

it.each([320,390,480,640])('keeps touch control rectangles clear of the compact hotbar at width %i', width => {
  const layout = touchControlLayout(width,400), hud = gameHudLayout(width,400);
  const controls = [layout.interactButton,layout.secondaryButton,layout.jumpButton,
    { x: layout.joystickCenter.x-layout.joystickRadius, y: layout.joystickCenter.y-layout.joystickRadius, width:layout.joystickRadius*2,height:layout.joystickRadius*2 }];
  for (const control of controls) for (const slot of [hud.hotbar, hud.craftingButton]) expect(control.x < slot.x+slot.width && control.x+control.width > slot.x
    && control.y < slot.y+slot.height && control.y+control.height > slot.y).toBe(false);
});
