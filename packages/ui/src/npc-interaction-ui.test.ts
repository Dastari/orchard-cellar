    ...
    for (const width of [320, 640])
        for (const scale of [1, 2, 3])
            for (const dpr of [1, 1.25]) {
                ...
                const close = ui.root.entries().find(entry => entry.element.kind === 'button' && entry.element.props['label'] === 'X')!.element;
