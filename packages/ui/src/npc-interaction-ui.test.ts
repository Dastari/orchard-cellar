...
describe('NPC interaction layout', () => {
  it('keeps the shop frame inside a compact 320x240 game UI', () => {
    ...
    expect(layout.currency.y + 12).toBeLessThan(layout.filter.y);
  ...
  it('uses a renamed live dialogue and its exact authored merchant frame', () => {
    ...
    const model = { width: 480, height: 270, npcId: 100n, dialogueId: 'moon_merchant', shopId: 'general_tools', nodeId: 'moon_shop',
  ...
  it('uses only active authored quest, item, and shop presentation when a registry is supplied', () => {
    const source = bootstrapContentRegistry();
    const moonItem = {
      ...
      maxStack: 7,
      economy: { buy: 123, sell: 45 },
    };
    const moonQuest = {
      ...source.quests.get('quest:marlow_important_book')!,
      id: 'quest:moon_errand' as const,
      title: 'Moon Errand',
      prerequisites: undefined,
      rewards: {
        bronze: 0, experience: [],
        items: [{ item: 'item:moon_ledger' as const, count: 2 }],
      },
    };
    const registry = buildContentRegistry([
      ...bootstrapContentRows(),
      { id: moonItem.id, kind: moonItem.kind, json: moonItem },
      { id: moonQuest.id, kind: moonQuest.kind, json: moonQuest },
      { id: 'shop:moon_market', kind: 'shop', json: {
        id: 'shop:moon_market', kind: 'shop', schemaVersion: 1,
        currency: { kind: 'bronze' }, offers: [{ item: moonItem.id }],
      } },
    ]).registry;
    const available = {
      id: 'moon_offer', label: 'Moon errand', nextNodeId: null,
      quest: { questId: 'moon_errand', requires: 'available' as const, action: 'accept' as const },
    };
    const turnIn = {
      ...available,
      quest: { questId: 'moon_errand', requires: 'complete' as const, action: 'turn_in' as const },
    };
    expect(dialogueChoiceIsAvailable(available, [], registry)).toBe(true);
    expect(dialogueChoiceIsAvailable(available, [], source)).toBe(false);
    expect(dialogueChoiceRewardTooltip(turnIn, registry)).toBe('REWARDS: MOON LEDGER ×2');
    ...
    ui.update({
      width: 480, height: 270, npcId: 2n, dialogueId: 'tool_merchant',
      shopId: 'moon_market', nodeId: 'shop', balanceBronze: 10_000n,
      inventory: [], contentRegistry: registry,
    });
    const layout = npcInteractionLayout(480, 270, true);
    const plus = { x: layout.list.x + layout.list.width - 20, y: layout.list.y + 10 };
    for (let count = 0; count < 8; count += 1) ui.pointerDown(plus, 0);
    expect(ui.shopState).toMatchObject({
      totalBronze: 861n,
      lines: [{ itemKind: 'moon_ledger', quantity: 7 }],
    });
  });

  it('does not revive compiled merchant price, stack, or availability on the live path', () => {
    const source = bootstrapContentRegistry();
    const rows = bootstrapContentRows().map((row) => {
      if (row.id === 'item:axe') {
        const axe = source.items.get(row.id)!;
        return { ...row, json: {
          ...axe, displayName: 'Moon Axe', maxStack: 2,
          economy: { ...axe.economy, buy: 17 },
        } };
      }
      if (row.id === 'shop:general_tools') {
        const shop = source.shops.get(row.id)!;
        return { ...row, json: { ...shop, offers: [{ item: 'item:axe' }] } };
      }
      return row;
    });
    ...
    const model = {
      width: 480, height: 270, npcId: 2n, dialogueId: 'tool_merchant',
      shopId: 'general_tools', nodeId: 'shop', balanceBronze: 10_000n,
      inventory: [], contentRegistry: registry,
    };
    ui.update(model);
    const layout = npcInteractionLayout(480, 270, true);
    const plus = { x: layout.list.x + layout.list.width - 20, y: layout.list.y + 10 };
    for (let count = 0; count < 3; count += 1) ui.pointerDown(plus, 0);
    expect(ui.shopState).toMatchObject({
      totalBronze: 34n,
      lines: [{ itemKind: 'axe', quantity: 2 }],
    });

    const unavailableRegistry = buildContentRegistry(rows.map((row) => {
      if (row.id !== 'item:axe') return row;
      const axe = registry.items.get(row.id)!;
      return { ...row, json: { ...axe, economy: { ...axe.economy, buy: null } } };
    })).registry;
    ui.update({ ...model, contentRegistry: unavailableRegistry });
    ui.pointerDown(plus, 0);
    expect(ui.shopState.lines).toEqual([]);

    const retiredShopRegistry = buildContentRegistry(rows.map((row) => {
      if (row.id !== 'shop:general_tools') return row;
      return { ...row, json: { ...registry.shops.get(row.id)!, retired: true } };
    })).registry;
    ui.update({ ...model, contentRegistry: retiredShopRegistry });
    ui.pointerDown(plus, 0);
    expect(ui.shopState.lines).toEqual([]);
  });
  ...
  it('fails closed for missing or retired live quests in every quest state', () => {
    ...
    expect(dialogueChoiceIsAvailable(retired,
      [{ questId: 'marlow_important_book', state: 'active' }], registry)).toBe(false);
    expect(dialogueChoiceIsAvailable(missing,
      [{ questId: 'absent_quest', state: 'complete' }], registry)).toBe(false);
    ...
    expect(dialogueChoiceRewardTooltip(turnIn, activeQuestRegistry)).toBe(
      'REWARDS: 1 GOLD / 100 EXPLORER XP / MARLOW BOOK ×1',
    );
  });
  ...
  it('starts every row at zero and submits multiple item kinds as one cart', async () => {
    ...
    ui.update({
      width: 480, height: 270, npcId: 2n, dialogueId: 'tool_merchant', shopId: 'general_tools', nodeId: 'shop',
  ...
  it('does not invent a general-tools shop for a dialogue without an authored shop', () => {
    const buy = vi.fn().mockResolvedValue(undefined);
    const ui = new NpcInteractionUi({} as UiSkin, {} as PixelUi, {} as never, {
    ...
    ui.update({
      width: 480, height: 270, npcId: 7n, dialogueId: 'tool_merchant', nodeId: 'shop',
      balanceBronze: 100_000n, inventory: [],
    });
    const layout = npcInteractionLayout(480, 270, true);
  ...
  it('swipe-scrolls Marlow\'s shop rows on touch screens', () => {
    ...
    ui.update({
      width: 320, height: 160, npcId: 2n, dialogueId: 'tool_merchant', shopId: 'general_tools', nodeId: 'shop',
