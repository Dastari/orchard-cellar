...
export function overworldUiLayout(width: number, height: number, options: OverworldUiLayoutOptions = {}): OverworldUiLayout {
  ...
  type SystemMenuAction = 'resume' | 'settings' | 'help' | 'developer' | 'fullscreen' | 'update' | 'exitDelve' | 'signOut' | 'quit';
...
export class OverworldUi {
  ...
  update(model: OverworldUiModel): void {
    ...
    this.statisticsScreen.update(model.statistics ?? { statistics: [] });
  ...
  pointerDown(point: UiPoint, button: number, modifiers: {
    ...
    if (this.openWindowValue === 'crafting' && this.selectedCraftingRecipeId !== null
      && (button === 0 || button === 2) && clickedCraftingRecipe === undefined) {
      this.selectedCraftingRecipeId = null;
    }
    if (button === 0) {
  ...
  private drawStatus(context: CanvasRenderingContext2D): void {
    ...
    if (hasEquippedWatch(this.model.inventory)) this.drawWatchStatus(context);
  }
  ...
  private activeWindowRect(): UiRect {
    ...
    if (this.openWindowValue === 'character' || this.openWindowValue === 'skills'
      || this.openWindowValue === 'statistics' || this.openWindowValue === 'quests') return this.layout.progressionWindow;
  ...
  private syncActiveWindow(): void {
    ...
    for (const slot of this.placeableItemSlots) slot.visible = false;
    ...
    effects.forEach((effect, index) => {
      ...
      const asset = effect.effectKind === 'winded' ? this.skin.effectWinded
        : effect.effectKind === 'orchard_tea' ? this.skin.effectOrchardTea
          : effect.effectKind === 'fruitful_energy' ? this.skin.effectFruitfulEnergy
  ...
  private drawInventory(context: CanvasRenderingContext2D, rect: UiRect): void {
    ...
    this.equipmentItemSlots.forEach((slot, visualIndex) => {
      ...
      const locked = definition.acceptedKinds.length === 0;
  ...
  private drawCrafting(context: CanvasRenderingContext2D, rect: UiRect): void {
    ...
    const previewPattern = this.selectedCraftingRecipeId === null ? null : craftingRecipePattern(
    ...
    this.craftingItemSlots.forEach((slot, index) => {
      ...
      if (slot.item) this.drawInventoryItem(context, slot.bounds, slot.item.itemKind, slot.item.quantity, slot.item.durability, slot.item.lit);
        ...
        const ghostKind = previewPattern?.[index] ?? null;
        if (ghostKind !== null) {
          context.save();
          context.globalAlpha *= 0.42;
          context.filter = 'grayscale(35%)';
          this.drawInventoryItem(context, slot.bounds, ghostKind, 1);
          context.restore();
        }
      }
    });
  ...
  private durabilityDefinition(itemKind: string) {
    return this.model.contentRegistry === undefined
      ? toolDurabilityDefinition(itemKind)
      : runtimeDurabilityDefinition(this.model.contentRegistry, itemKind);
  }
