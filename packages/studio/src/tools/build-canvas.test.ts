...
describe('canvas-native Build and asset tools', () => {
  it.each(['/build/map', '/build/map/terrain-lab', '/build/map/procedural-world'])('builds map route %s without route controls', async (path) => {
    ...
    expect(surface.actions.map(({ id }) => id).filter((id) => id.startsWith('map-left-view-')))
      .toEqual(['map-left-view-palette', 'map-left-view-world', 'map-left-view-live']);
    ...
    if (path === '/build/map') {
      surface.actions.find(({ id }) => id === 'map-left-view-world')?.activate();
      ...
      expect(outlinerSurface.actions.some(({ id }) => id.startsWith('map-outliner-select-map-object:')))
        .toBe(true);
      outlinerSurface.actions.find(({ id }) => id === 'map-left-view-live')?.activate();
      ...
      liveOutlinerSurface.actions.find(({ id }) => id === 'map-left-view-palette')?.activate();
      ...
      surface.actions.find(({ id }) => id === 'map-workspace-biomes')?.activate();
      const biomeSurface = buildMapCanvasTool(toolContext);
      expect(biomeSurface.nodes.some(({ id }) => id === 'map-object-search')).toBe(true);
      expect(biomeSurface.nodes.some(({ id }) => id.startsWith('map-biome-'))).toBe(true);
      expect(biomeSurface.nodes.some(({ id }) => id.startsWith('map-terrain-tool-'))).toBe(false);
      ...
      surface.actions.find(({ id }) => id === 'map-workspace-scatter')?.activate();
      const scatterSurface = buildMapCanvasTool(toolContext);
  ...
  it('mounts searchable keyboard Outliners and guarded one-entry authored tree edits', () => {
    ...
    surface.actions.find(({ id }) => id === 'map-left-view-live')?.activate();
    surface = buildMapCanvasTool(toolContext);
  ...
  it('provides compact canvas-native layer rename/reorder controls with locked system boundaries', () => {
    ...
    expect(surface.nodes.find(({ id }) => id === rename?.id)).toMatchObject({ glyph: 'key_r', label: undefined });
