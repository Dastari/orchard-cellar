...
export interface QuestProgressFixture {
  ...
  readonly equipment?: Readonly<Partial<Record<'weapon' | 'body', boolean>>>;
  readonly furnishings?: Readonly<Partial<Record<'seat' | 'table' | 'lamp' | 'rug', number>>>;
}
  ...
  questPreview(id: string, fixture: QuestProgressFixture = {}): QuestPreview {
    ...
    const objectives = definition.objectives.map((objective) => {
      ...
      const required = objective.kind === 'collect'
        ...
        : objective.kind === 'location' || objective.kind === 'equipment' || objective.kind === 'furnishing' ? 1 : objective.count;
      ...
      else if (objective.kind === 'equipment') current = fixture.equipment?.[objective.category] ? 1 : 0;
      else if (objective.kind === 'furnishing') current = fixture.furnishings?.[objective.category] ?? 0;
