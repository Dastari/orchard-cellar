...
function terrainHitAtElevation(
  ...
  const projection = (elevation - baseDatum)
    * terrainVisualProjectionRowsPerLevel(terrain) * TILE_SIZE_PIXELS;
...
export function topmostEditorTerrainHit(
  ...
  for (
    ...
    elevation >= Math.min(terrainMinimumElevation(terrain), baseDatum);
    elevation -= 1
  ) {
    ...
    candidates.push({
      hit,
      footY: (hit.tileY + 1 - (elevation - baseDatum) * visualProjectionRows)
        * TILE_SIZE_PIXELS,
  ...
  for (
    ...
    const sourceTileY = projectedTileY + (contourLevel - baseDatum) * visualProjectionRows;
