...
describe("offline editor elevated terrain picking", () => {
  it("selects the logical source tile of a projected elevated surface", () => {
    ...
    // Two levels project row 4 upward by two visual rows, onto screen row 2.
    expect(topmostEditorTerrainHit(terrain, 3 * 16 + 8, 2 * 16 + 8)).toEqual({
      tileX: 3,
      tileY: 4,
      elevation: 2,
    });
  ...
  it("reverses the downward projection of a signed pit surface", () => {
    const terrain = terrainWithElevations(8, 8, [2, 3, 4, 5, 6].map(
      (tileY) => [3, tileY, -2] as const,
    ));
    expect(topmostEditorTerrainHit(terrain, 3 * 16 + 8, 4 * 16 + 8)).toEqual({
      tileX: 3,
      tileY: 2,
      elevation: -2,
    });
  ...
  it("selects a visible projected cliff face with no surface in its screen cell", () => {
    ...
    // (4, 2), whose walkable surface is L0. It is still selectable as the L1
  ...
  it("locks subsequent stroke samples to the elevation chosen at pointer-down", () => {
    ...
    expect(editorTerrainHitOnPlane(terrain, 3 * 16 + 8, 1 * 16 + 8, 1)).toEqual({
      tileX: 3,
      tileY: 2,
      elevation: 1,
    });
  ...
  it("uses the visible flat chunk when an ungenerated elevated apron is under the pointer", () => {
    ...
    ).toEqual({ tileX: 3, tileY: 13, elevation: 0 });
    expect(
      proceduralEditorTerrainHit(
        terrain,
        projectedX,
