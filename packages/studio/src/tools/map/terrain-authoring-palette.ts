...
const BOOTSTRAP_TILESET_RESOLVER = runtimeTilesetResolver(BOOTSTRAP_TILESET_DEFINITIONS);
...
export function terrainSurfaceFamilyChoices(search = ''): readonly TerrainSurfaceFamilyChoice[] {
  return TERRAIN_SURFACE_FAMILY_IDS.map((familyId) => {
...
function cliffPreview(definition: TilesetContentDefinition): TerrainPalettePreview {
  ...
  throw new TypeError(`Bootstrap tileset ${definition.id} has no previewable authored frame`);
}

/** Only immutable bootstrap definitions are publishable choices. */
export function terrainCliffFamilyChoices(search = ''): readonly TerrainCliffFamilyChoice[] {
  return BOOTSTRAP_TILESET_DEFINITIONS.map((definition) => ({
...
export function terrainCliffFamilyExclusions(): readonly TerrainCliffFamilyExclusion[] {
  return RESERVED_TILESET_FAMILIES.map(({ familyId, reason }) => ({ familyId, reason }));
}
...
export function exactTerrainOverrideChoicesAt(
  ...
  const { document, tileX, tileY, query, familyId } = request;
  if (!mapCoordinateInBounds(document, tileX, tileY)) return [];
  const compiled = compileMapDocument(document, BOOTSTRAP_TILESET_RESOLVER);
  ...
  for (const layer of trace.layers) {
    ...
    for (const definition of BOOTSTRAP_TILESET_DEFINITIONS) {
      if (familyId !== undefined && definition.familyId !== familyId) continue;
      const tileSet = BOOTSTRAP_TILESET_RESOLVER.tileSetFor(definition.familyId);
