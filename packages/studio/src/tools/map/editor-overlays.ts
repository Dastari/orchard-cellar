...
export function drawEditorWorldOverlays(
  ...
  const authoredTransitions = options.proceduralChunks === undefined
    ? [
        ...document.transitions,
        ...(document.stairRuns ?? []).flatMap(expandStairRun),
      ]
    : [];
  const automaticTransitions = options.proceduralChunks?.terrain.terrainTransitions ?? [];
  const transitionOffsetX = options.proceduralChunks?.compositionMinTileX ?? 0;
  const transitionOffsetY = options.proceduralChunks?.compositionMinTileY ?? 0;
  for (const [authored, transitions] of [
    [true, authoredTransitions],
    [false, automaticTransitions],
  ] as const) {
    for (const transition of transitions) {
      const lowerTileX = transition.lowerTileX + transitionOffsetX;
      const lowerTileY = transition.lowerTileY + transitionOffsetY;
      const upperTileX = transition.upperTileX + transitionOffsetX;
      const upperTileY = transition.upperTileY + transitionOffsetY;
      if (Math.max(lowerTileX, upperTileX) < minimumX
        || Math.min(lowerTileX, upperTileX) > maximumX
        || Math.max(lowerTileY, upperTileY) < minimumY
        || Math.min(lowerTileY, upperTileY) > maximumY) continue;
      const [lowerX, lowerY] = screenTile(
        options.terrain,
        lowerTileX,
        lowerTileY,
      ...
      const [upperX, upperY] = screenTile(
        options.terrain,
        upperTileX,
        upperTileY,
      ...
      context.strokeStyle = authored
        ? transition.kind === "stairs" ? "#f1a1ff" : "#ffd36b"
        : "#52d8ee";
      context.fillStyle = context.strokeStyle;
      context.lineWidth = authored ? 2 : 1;
      context.setLineDash(authored ? [] : [3, 2]);
      context.beginPath();
      context.moveTo(lowerX + 8 * scale, lowerY + 8 * scale);
      context.lineTo(upperX + 8 * scale, upperY + 8 * scale);
      context.stroke();
      context.fillRect(upperX + 6 * scale, upperY + 6 * scale, 4 * scale, 4 * scale);
    }
  }
  context.setLineDash([]);
  context.restore();
