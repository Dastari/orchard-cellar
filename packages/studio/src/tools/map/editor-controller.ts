...
export function mapEditorLiveMarkers(liveRows: StudioLiveRows | null, registry: Pick<ContentRegistry, 'objects'> | null = null): readonly MapEditorLiveMarker[] {
  ...
  for (const row of liveRows.npcs) {
    ...
    markers.push({ id: row.id.toString(), entityKind: 'npc', kind: row.species ?? row.kind,
...
export class MapEditorController {
  ...
  restoreSession(state: MapEditorSessionRestore): boolean {
    ...
    if (!Number.isFinite(state.camera.x) || !Number.isFinite(state.camera.y)
      ...
      || !TERRAIN_SURFACE_FAMILY_IDS.includes(selectedSurfaceFamily)
      || !terrainCliffFamilyChoices().some(({ familyId }) => familyId === selectedCliffFamily)
  ...
  selectSurfaceFamily(family: TerrainSurfaceFamilyId): boolean {
    if (!TERRAIN_SURFACE_FAMILY_IDS.includes(family)) return false;
  ...
  selectCliffFamily(family: string): boolean {
    if (!terrainCliffFamilyChoices().some(({ familyId }) => familyId === family)) return false;
  ...
  exactTerrainChoicesAtSelection(query = ''): readonly ExactTerrainOverrideChoice[] {
    ...
    return point === null ? [] : exactTerrainOverrideChoicesAt({
      ...
      tileX: point.tileX, tileY: point.tileY, query,
    });
  }
  ...
  private armedTerrainPatch(): Readonly<MapCellPatch> | null {
    ...
    if (this.#terrainPaletteMode === 'surface_family') return {
      surface: 'grass', feature: 'none', surfaceFamily: this.#selectedSurfaceFamily,
    };
    if (this.#terrainPaletteMode === 'cliff_family') return {
      cliffFamily: this.#selectedCliffFamily,
    };
