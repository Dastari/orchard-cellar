...
function drawMapGameplayAnchorMarkers(
  ...
  for (const anchor of anchors) {
    ...
    const tooltip = `${anchor.kind.toUpperCase()} · ${anchor.id} · RUNTIME UNBOUND`;
    const tooltipWidth = Math.min(280, Math.max(150, tooltip.length * 7 + 14));
    const tooltipBounds = {
      x: Math.max(viewport.x + 4, Math.min(
        viewport.x + viewport.width - tooltipWidth - 4,
        Math.round(point.x - tooltipWidth / 2),
      )),
      y: Math.max(viewport.y + 4, Math.round(point.y - radius - 32)),
      width: tooltipWidth,
      height: 25,
    };
    context.fillStyle = STUDIO_SKIN_TOKENS.parchmentLight;
    context.fillRect(tooltipBounds.x, tooltipBounds.y, tooltipBounds.width, tooltipBounds.height);
    context.strokeStyle = STUDIO_SKIN_TOKENS.woodDark;
    context.lineWidth = 2;
    context.strokeRect(tooltipBounds.x, tooltipBounds.y, tooltipBounds.width, tooltipBounds.height);
    drawPixelTextInRect(context, art.fonts, tooltip, tooltipBounds, {
      color: STUDIO_SKIN_TOKENS.ink,
...
export class MapEditorRenderer {
  ...
  readonly #groundCache = new GroundChunkCache();
  readonly #generatedBaseGroundCache = new GroundChunkCache();
  ...
  constructor(private readonly invalidate: () => void = () => undefined) {
  ...
  private terrainFor(mapDocument: MapDocumentV3, terrainIdentity: object): TerrainArray | null {
    ...
    if (this.#terrainDerivationTimer !== null) clearTimeout(this.#terrainDerivationTimer);
    this.#terrainDerivationTimer = null;
    const pending = this.#terrainLoader.load(mapDocument);
    if (pending !== null) {
    ...
    this.#terrainFallbackTimer = setTimeout(() => {
      ...
      if (this.#disposed || this.#terrainPendingIdentity !== terrainIdentity) return;
      try {
        this.acceptTerrain(mapDocument, terrainIdentity, buildMapEditorTerrain(mapDocument));
        this.invalidate();
      } catch (error) {
    ...
    this.#terrainDerivationTimer = setTimeout(() => {
      ...
      try {
        this.acceptTerrainDerivatives(
          terrainIdentity,
          buildMapEditorTerrainDerivatives(mapDocument, terrain),
        );
        this.invalidate();
      } catch (error) {
  ...
  private liveMarkerHasArtwork(marker: MapEditorLiveMarker): boolean {
    return marker.entityKind !== 'npc' || marker.kind !== 'bee' || marker.activity !== 'inside_hive';
  }
  ...
  private enqueueLiveMarker(
    ...
    const draw = (): void => {
      ...
      if (marker.entityKind === 'npc') {
        ...
        if (marker.kind === 'boat') {
        ...
        } else if (marker.kind === 'horse') {
          drawOverworldHorse(
            context, art, worldX, worldY, facing, moving, animationFrame,
            camera.x, camera.y, camera.zoom, false, undefined, marker.variant ?? 0,
            marker.activity,
          );
        } else if (isWildlifeSpecies(marker.kind)) {
          drawOverworldWildlife(
            context, art, marker.kind, marker.variant ?? 0, marker.activity ?? 'idle',
            worldX, worldY, facing, moving, animationFrame, camera.x, camera.y, camera.zoom,
          );
        } else if (art.rogueEnemies[marker.kind] !== undefined) {
          drawOverworldRogueEnemy(
            context, art, marker.kind, marker.health === 0 ? 'defeated' : marker.activity ?? 'idle',
            worldX, worldY, facing, moving, animationFrame, camera.x, camera.y, camera.zoom,
            false,
          );
        } else {
          drawOverworldMerchant(
            context, art, worldX, worldY, facing, moving, animationFrame,
            camera.x, camera.y, camera.zoom, marker.kind, marker.activity,
          );
        }
        return;
      }
