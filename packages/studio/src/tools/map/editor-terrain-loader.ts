...
export class MapEditorTerrainLoader {
  ...
  load(document: MapDocumentV3): Promise<MapEditorTerrainLoadResult> | null {
  ...
  private worker(): Worker | null {
    ...
    try {
      ...
      this.#worker.addEventListener('message', (event: MessageEvent<MapEditorTerrainWorkerResponse>) => {
        ...
        void decodeMapEditorTerrainAsync(event.data.terrain, shouldContinue).then(async (terrain) => ({
          terrain,
          derivatives: await decodeMapEditorTerrainDerivativesAsync(
