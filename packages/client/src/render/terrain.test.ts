...
describe('shared client terrain array', () => {
  ...
  it('maps organic plateaus and their ramps onto the authored raised stone-cliff topology', () => {
    ...
    const role = (x: number, y: number, value: typeof SURVIVAL_CLIFF_ROLES[number]): void => {
      terrain.cliffRoles[y * terrain.width + x] = SURVIVAL_CLIFF_ROLES.indexOf(value);
    };
    for (let tileY = 1; tileY <= 3; tileY += 1) {
      for (let tileX = 1; tileX <= 3; tileX += 1) terrain.plateaus[tileY * terrain.width + tileX] = 1;
    }
    role(2, 1, 'top');
    role(1, 2, 'left');
    role(3, 2, 'right');
  ...
  it('layers a projected wall and inverse corner behind a continuing plateau step', () => {
    ...
    terrain.cliffRoles[index(1, 1)] = SURVIVAL_CLIFF_ROLES.indexOf('bottom_left');
    terrain.cliffRoles[index(2, 2)] = SURVIVAL_CLIFF_ROLES.indexOf('left');
    terrain.cliffRoles[index(2, 3)] = SURVIVAL_CLIFF_ROLES.indexOf('bottom_left');
    ...
    terrain.cliffRoles[index(2, 2)] = SURVIVAL_CLIFF_ROLES.indexOf('bottom_left');
    expect(plateauForegroundFrameIndicesAt(terrain, 2, 1)).toEqual([1]);
  ...
  it('pairs outward side steps with their mirrored lower inverse corners', () => {
    ...
    const index = (x: number, y: number): number => y * terrain.width + x;
    const setRole = (x: number, y: number, role: typeof SURVIVAL_CLIFF_ROLES[number]): void => {
  ...
  it('keeps every generated inverse-corner overlay on walkable plateau terrain', () => {
    ...
    for (let tileY = 0; tileY < terrain.height; tileY += 1) {
      for (let tileX = 0; tileX < terrain.width; tileX += 1) {
        ...
        for (const frame of frames) frameCounts[frame] = (frameCounts[frame] ?? 0) + 1;
  ...
  it('derives the existing world edge and ramp semantics from plateau occupancy', () => {
    ...
    for (let tileY = 0; tileY < terrain.height; tileY += 1) {
      for (let tileX = 0; tileX < terrain.width; tileX += 1) {
        ...
        const generatedRole = SURVIVAL_CLIFF_ROLES[terrain.cliffRoles[index] ?? 0] ?? 'none';
  ...
  it('maps shallow dirt terraces onto the same connected topology with their own ramp', () => {
    ...
    for (let tileY = 1; tileY <= 3; tileY += 1) {
      for (let tileX = 1; tileX <= 3; tileX += 1) terrain.dirtTerraces[tileY * terrain.width + tileX] = 1;
    }
    expect(dirtTerraceFrameIndexAt(terrain, 2, 2)).toBe(46);
    const setRole = (x: number, y: number, role: typeof SURVIVAL_DIRT_CLIFF_ROLES[number]): void => {
      terrain.dirtCliffRoles[y * terrain.width + x] = SURVIVAL_DIRT_CLIFF_ROLES.indexOf(role);
    };
    setRole(1, 2, 'ramp_top_left'); setRole(2, 2, 'ramp_top_right');
    setRole(1, 3, 'ramp_bottom_left'); setRole(2, 3, 'ramp_bottom_right');
  ...
  it('selects grass-edged freshwater and authored waterfall strips', () => {
    ...
    expect(freshwaterFrameIndexAt(innerCorner, 1, 1)).toBe(13);
    innerCorner.biomes[0] = 2;
    innerCorner.biomes[2] = 4;
    expect(freshwaterFrameIndexAt(innerCorner, 1, 1)).toBe(12);
    innerCorner.biomes[2] = 2;
    innerCorner.biomes[6] = 4;
    expect(freshwaterFrameIndexAt(innerCorner, 1, 1)).toBe(10);
    innerCorner.biomes[6] = 2;
    innerCorner.biomes[8] = 4;
    expect(freshwaterFrameIndexAt(innerCorner, 1, 1)).toBe(9);
  ...
  it('selects one animated waterfall overlay and keeps animated rocks off banks', () => {
    ...
    for (let tileY = 2; tileY < 7; tileY += 1) {
      for (let tileX = 2; tileX < 5; tileX += 1) falls.biomes[tileY * falls.width + tileX] = 3;
    }
