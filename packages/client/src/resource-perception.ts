...
export type DetectedOre = {
  ...
  readonly oreKind: SurvivalOreKind;
...
export class ResourcePerceptionCache {
  ...
  project(input: ResourcePerceptionInput): ResourcePerceptionProjection {
    ...
    const projectOre = (tileX: number, tileY: number, oreKind: SurvivalOreKind): DetectedOre => (
    ...
    for (const resource of resources) {
      ...
      if (ability.minimapOre && oreMapRadius > 0 && distance <= oreMapRadius ** 2 && isMineableOreKind(resource.kind)) {
        minimapOre.set(tileY * terrain.width + tileX, projectOre(tileX, tileY, resource.kind));
      }
      if (ability.minimapFishing && fishRadius > 0 && distance <= fishRadius ** 2 && resource.kind === 'fish_pool') {
