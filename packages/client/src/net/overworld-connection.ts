...
const WORLD_REGION_RANGE_QUERIES = 16;
const ROGUE_REGION_RANGE_QUERIES = 5;
...
function isUnifiedChest(row: Pick<WorldPlaceable, 'kind' | 'definitionId'>): boolean {
...
export function chestLifecycleTargetKind(
  ...
  return placeable?.id === chestId && isUnifiedChest(placeable)
    ? 'placeable'
    : 'chest';
}
...
export class OverworldConnection {
  ...
  private releaseConnection(): void {
    ...
    this.rogueRunUpgrades.clear(); this.rogueEnemyProfiles.clear();
    ...
    this.playerStatistics.clear(); this.skillTracks.clear(); this.skillNodes.clear(); this.questWorldItems.clear();
  ...
  chestTargetKind(chestId: bigint): 'placeable' | 'chest' {
    return chestLifecycleTargetKind(chestId, this.chestPlaceableSources.get(chestId));
  }
  ...
  private subscribeRegion(connection: DbConnection, position: PlayerPosition, force = false): void {
    ...
    const coreQueries = rogueInstance
      ? [positions, playerJumps, projectiles, npcs, rogueEnemyProfiles, ...discoveryQueries]
      : [
          positions, playerJumps, resources, soil, crops, worldItems, projectiles, combatTargets,
          placeables, npcs, wildlifeProfiles, rogueEnemyProfiles, hives, surfaces, ...discoveryQueries,
        ];
  ...
  private bindTableEvents(connection: DbConnection): void {
    ...
    connection.db.contentDefinition.onInsert(() => this.scheduleContentRefresh(connection));
    connection.db.contentDefinition.onUpdate(() => this.scheduleContentRefresh(connection));
    connection.db.contentDefinition.onDelete(() => this.scheduleContentRefresh(connection));
    ...
    connection.db.worldPlaceable.onUpdate((context, old, row) => resource(context.event.id, () => {
      if (isUnifiedChest(row)) {
        ...
        if (isUnifiedChest(old)) this.chests.delete(old.id);
      }
    }));
    ...
    const setActivePlaceable = (row: WorldPlaceable): void => {
      ...
      this.activeChest = isUnifiedChest(row) ? compatibilityChest(row) : null;
      if (!isUnifiedChest(row)) this.openChestSlots.clear();
    };
    ...
    const setOpenPlaceableSlot = (row: WorldPlaceableSlot): void => {
      ...
      if (this.activePlaceable !== null && isUnifiedChest(this.activePlaceable)) {
  ...
  private clearSpaceScopedCaches(): void {
    ...
    this.wildlifeProfiles.clear(); this.rogueEnemyProfiles.clear(); this.hives.clear(); this.worldSpeech.clear();
  ...
  private refreshContent(connection: DbConnection): void {
    ...
    const rows = [...connection.db.contentDefinition.iter()] as ContentDefinition[];
  ...
  private hydrateSelf(connection: DbConnection): void {
    ...
    this.activeChest = this.activePlaceable !== null && isUnifiedChest(this.activePlaceable)
