...
it('explains picking, growth and ripening in the same keyboard/touch prompt', () => {
  ...
  expect(orchardHarvestPrompt(registry, { ...tree, growthStage: 1 }, 0n)).toContain('WHEN MATURE');
  expect(orchardHarvestPrompt(registry, { ...tree, depleted: true }, 0n)).toBe('TREE REGROWING');
