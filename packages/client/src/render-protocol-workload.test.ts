...
function capture(overrides: Partial<ProtocolWorkloadFrame> = {}, count = 300) {
  ...
  for (let i = 0; i < 120; i++) buffer.record({ ...identity, assetSeason: 'summer', cameraElapsedMs: i, cameraX: 0, cameraY: 0,
...
it('compares the same route clock across different frame cadence and warm-up boundary offsets', () => {
  const fast = capture(), slow = capture();
  const route = (time: number) => time * 0.032;
  fast.cameraElapsedMs = fast.cameraElapsedMs.map(t => t * 16.667 + 5016.667);
  slow.cameraElapsedMs = slow.cameraElapsedMs.map(t => t * 33.333 + 5033.333);
  fast.cameraX = fast.cameraElapsedMs.map(route); slow.cameraX = slow.cameraElapsedMs.map(route);
  expect(compareProtocolWorkloads(fast, slow).comparable).toBe(true);
  slow.cameraX = slow.cameraX.map(x => x + 2);
  expect(compareProtocolWorkloads(fast, slow).issues).toContain('different_measured_cameraX_route');
});
