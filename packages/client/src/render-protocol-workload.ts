...
export interface ProtocolWorkloadFrame {
  ...
  readonly cameraElapsedMs: number;
...
export class ProtocolWorkloadBuffer {
  ...
  record(frame: ProtocolWorkloadFrame, renderItems: number, timestamp = this.count): void {
    ...
    this.values[offset++] = frame.cameraElapsedMs;
  ...
  report() {
    ...
    return { identity: this.identity, qualified: issues.length === 0, issues, frameCount: this.count,
      ...
      timestamps: column(9), cameraElapsedMs: column(10), cameraX: column(0), cameraY: column(1), playerX: column(2), playerY: column(3) };
  }
}
...
export function compareProtocolWorkloads(left: ReturnType<ProtocolWorkloadBuffer['report']>,
  ...
  // Compare the actual route clock over the common sample window. Normalizing
  // each capture by its first/last frame shifts the path when cadence differs.
  const first = Math.max(left.cameraElapsedMs[0] ?? NaN, right.cameraElapsedMs[0] ?? NaN);
  const last = Math.min(left.cameraElapsedMs.at(-1) ?? NaN, right.cameraElapsedMs.at(-1) ?? NaN);
  const sample = (report: typeof left, key: 'cameraX' | 'cameraY', fraction: number) => {
    if (!(last > first)) return Number.NaN;
    const time = first + (last - first) * fraction;
    let i = 1;
    while (i < report.frameCount - 1 && report.cameraElapsedMs[i]! < time) i++;
    const a = report.cameraElapsedMs[i - 1]!, b = report.cameraElapsedMs[i]!;
