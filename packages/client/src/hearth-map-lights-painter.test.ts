...
import {liveMapObjectPointLights} from '@orchard/engine/live-map-runtime';
import {TOPSIDE_SPACE_ID} from '@orchard/sim';
import type {PointLight} from '@orchard/engine/lighting';
vi.mock('@orchard/engine/live-map-runtime',()=>({liveIslandDocument:()=>null,liveMapObjectPointLights:vi.fn()}));
it('collects offscreen map-light influence with contact projection only on the active visible topside',()=>{
  ...
  vi.mocked(liveMapObjectPointLights).mockReturnValue([source,{...source,worldX:200}]);
  ...
  const input={dynamicLighting:true,debugEntitiesHidden:false,
    ...
    snapshot:{content:{registry:{}},placeables:[],homesteads:[],liveMapDocument:null,clock:{authorityTick:42n}},
  ...
  for(const state of ['interior','hidden','lighting-off'] as const){
    lights.length=0;vi.mocked(liveMapObjectPointLights).mockClear();
    ...
    render();expect(lights,state).toEqual([]);expect(liveMapObjectPointLights,state).not.toHaveBeenCalled();
  }
});
