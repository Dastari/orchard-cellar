...
describe('quiet action input preflight', () => {
  const registryWithNode = (id: string, capabilities: readonly string[] = [], retired = false) => {
    const definition = {
      id: 'skill_tree:input_test', kind: 'skill_tree', schemaVersion: 1, track: 'explorer', levelCap: 50,
      ...(retired ? { retired: true } : {}),
      nodes: [
        { id: 'root', iconAsset: 'root', track: 'explorer', name: 'Root', description: 'Root',
          position: [0, 0], connects: [id], maxRank: 0, pointCost: 0, root: true, implemented: true },
        { id, iconAsset: 'skill', track: 'explorer', name: 'Skill', description: 'Skill',
          position: [1, 0], connects: ['root'], prerequisites: ['root'], maxRank: 3, pointCost: 1,
          implemented: true, ...(capabilities.length === 0 ? {} : { capabilities }) },
      ],
    };
    return buildContentRegistry([{
      id: definition.id,
      kind: definition.kind as 'skill_tree',
      json: definition,
    }]).registry;
  };
  ...
  it('uses the active authored mount jump skill rather than a fixed skill or NPC id', () => {
    ...
    const registry = registryWithNode('authored_high_jump');
    expect(jumpInputSkillAvailable([{ nodeId: 'surefooted', rank: 1 }], mount, true, registry)).toBe(false);
    expect(jumpInputSkillAvailable([{ nodeId: 'authored_high_jump', rank: 1 }], mount, true, registry)).toBe(true);
  ...
  it('uses renamed active foot-jump capabilities and rejects retired or missing definitions', () => {
    const active = registryWithNode('vaulting', ['foot_gap_jump']);
    const ranks = [{ nodeId: 'vaulting', rank: 2 }];
    expect(jumpInputSkillAvailable(ranks, null, false, active)).toBe(true);
    expect(jumpInputSkillAvailable(ranks, null, false, registryWithNode('vaulting', ['foot_gap_jump'], true)))
      .toBe(false);
    expect(jumpInputSkillAvailable(ranks, null, false, registryWithNode('different', ['foot_gap_jump'])))
      .toBe(false);
  });
});
