...
const derivedSources: Readonly<Record<string, string>> = {
  fishing_endurance: 'item_cf_fishing_rod',
};
    ...
    await Promise.all(iconFiles.map(async (fileName) => {
      ...
      expect(source.tags).toEqual(expect.arrayContaining([
        ...
        derivedSource === undefined
          ? source.sourcePath?.includes('/kenmi/') ? 'source.kenmi' : 'source.clockwork_raven'
          : `source.asset.${derivedSource}`,
      ]));
      if (nodeId === 'farmcraft') {
        expect(source.sourcePath).toBe('references/art/kenmi/cute-fantasy/icons/Cute_Fantasy_Icons_Tools/16x16/Tools_all_16x16.png');
        expect(source.sourceRegion).toEqual([0, 80, 16, 16]);
      }
