...
for (const [name, relativeSource] of parts) {
  ...
  const asset: AssetSource = {
    name,
    category: 'characters',
    ...(name === 'action_cf_base' ? { bakedShadowColor: '#00000064' } : {}),
    size: [WIDTH, HEIGHT],
    anchor: [16, 39],
    frames,
    frameKinds: Object.fromEntries(animationNames.map((animation) => [animation, 'animation'])),
    animationFps: Object.fromEntries(animationRows.map((entry) => [entry.name, entry.name.startsWith('sitting_') ? 2 : 10])),
...
console.log(`Extracted ${parts.length} modular character sword/hold/action layers from canonical rows 6-52.`);
