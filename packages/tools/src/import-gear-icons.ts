/** Import the reviewed weapon and structure-component icons listed in
 * gear-icon-imports.json from the licensed Kenmi icon sheets.
 *   npx tsx packages/tools/src/import-gear-icons.ts
...
type Entry = { item: string; asset: string; source: string; crop: [number, number, number, number]; sha256: string; group: string };
const manifest = JSON.parse(await readFile(new URL('packages/tools/src/gear-icon-imports.json', workspaceRoot), 'utf8')) as { imports: Entry[] };
const hashes = new Map<string, string>();
for (const entry of manifest.imports) {
  if (!entry.source.startsWith('references/art/kenmi/cute-fantasy/icons/')) throw new Error(`Not a licensed Kenmi icon sheet: ${entry.asset}`);
  if (!/^icon_(weapon|part)_[a-z0-9_]+$/.test(entry.asset)) throw new Error(`Invalid asset name: ${entry.asset}`);
...
for (const entry of manifest.imports) {
  ...
  Object.assign(asset, {
    ...
    tags: ['ui.icon', entry.group === 'weapon' ? 'item.weapon' : 'item.material', 'source.kenmi'],
...
console.log(`Imported ${manifest.imports.length} weapon and component icons`);
