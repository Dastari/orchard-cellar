...
describe('content pack measurement', () => {
  it('is deterministic and remains beneath the initial subscription envelope budget', () => {
    ...
    expect(first.runtimePayloadBytes).toBeLessThanOrEqual(CONTENT_INITIAL_PAYLOAD_BUDGET_BYTES);
    expect(first.runtimePayloadBytes).toBeLessThan(first.rowEnvelopeJsonBytes);
