...
const floorMiddle = decodePng(await readFile(resolve(root, 'references/art/kenmi/cute-fantasy/core/Tiles/Cave/Cave_Floor_Middle.png')));
const floor1 = decodePng(await readFile(resolve(root, 'references/art/kenmi/cute-fantasy/core/Tiles/Cave/Cave_Floor_1.png')));
const floor2 = decodePng(await readFile(resolve(root, 'references/art/kenmi/cute-fantasy/core/Tiles/Cave/Cave_Floor_2.png')));
const floorDecoration = decodePng(await readFile(resolve(root, 'references/art/kenmi/cute-fantasy/core/Tiles/Cave/Cave_Floor_Decoration.png')));
const walls = decodePng(await readFile(resolve(root, 'references/art/kenmi/cute-fantasy/core/Tiles/Cave/Cave_Walls.png')));
const supports = decodePng(await readFile(resolve(root, 'references/art/kenmi/cute-fantasy/core/Tiles/Cave/Cave_Wall_Support.png')));
...
for (let localY = 0; localY < previewTiles; localY += 1) {
  for (let localX = 0; localX < previewTiles; localX += 1) {
    ...
    if (decoration !== null) {
      copy(floorDecoration, decoration * tileSize, 0, localX, localY);
      continue;
    }
  }
}
...
const planAt = (x: number, y: number) => resolveRaisedTerrainContoursAt(
  elevationAt, 1, CAVE_RAISED_CLIFF_TILE_SET, 'tall', x, y,
)[0]?.plan;
// Rock is raised terrain: its opaque surface, rims and both face courses are
// displaced north by the wall height, exactly as the client draws them. The
// two logical floor rows under that displacement are the walk-behind band.
for (let localY = 0; localY < previewTiles + projectionRows; localY += 1) {
  for (let localX = 0; localX < previewTiles; localX += 1) {
    const tileX = originX + localX;
    const tileY = originY + localY;
    const projectedLocalY = localY - projectionRows;
    const plan = planAt(tileX, tileY);
    if (!dugAt(tileX, tileY) && (plan === undefined || plan.edgeFrame === null)) {
      copyUndug(localX, projectedLocalY);
    }
    if (!plan) continue;
    ...
    if (directLowerWall !== undefined && caveWallSupportAnchorAt(
      (x, y) => planAt(x, y)?.faceLayers.some((face) => face.direct && face.rowId === 'lower_wall') === true,
      tileX,
      tileY,
    )) {
