...
describe('authored Delve boon source authority',()=>{
  it('keeps the boon catalog out of runtime source and threads active registries through every consumer',()=>{
    ...
    const retained=read('../../ui/src/game-host/overlays.ts');
    const production=[sim,world,client,ui,retained].join('\n');
    ...
    const update=client.slice(client.indexOf('delveRewards.update('),client.indexOf('canvas.classList.toggle',client.indexOf('delveRewards.update(')));
    expect(update).toContain('offers: [...snapshot.rogueRewardOffers], registry: snapshot.content.registry');
    expect(retained).toContain('rogueUpgradeDefinition(model.registry, offer.upgradeId)');
    expect(client).not.toContain('drawRogueRewardOverlay(');
