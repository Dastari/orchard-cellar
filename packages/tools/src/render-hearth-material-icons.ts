...
const root=resolve(import.meta.dirname,'../../..'), width=960, height=120, scale=4, baseline=80;
...
for(const [index,name] of ['basalt','ashwood','cinder_ore','emberglass','guardian_seal'].entries()) {
  const path=`packages/assets/ui/icon_material_${name}.sprite.json`, source=await readFile(resolve(root,path),'utf8');
  ...
  manifest.push({index,name,path,sourcePath:asset.sourcePath,sourceRegion:asset.sourceRegion,anchor:asset.anchor,
    sha256:createHash('sha256').update(source).digest('hex')});
}
const png=encodePng(width,height,pixels), out=resolve(root,'output/doc60'); await mkdir(out,{recursive:true});
await writeFile(resolve(out,'hearth-material-icons.png'),png);
await writeFile(resolve(out,'hearth-material-icons.provenance.json'),JSON.stringify({draft:true,scale,baseline,
  order:'basalt, ashwood, cinder ore, emberglass, guardian seal',assets:manifest,imageSha256:createHash('sha256').update(png).digest('hex'),
  reproduce:'npx tsx packages/tools/src/render-hearth-material-icons.ts'},null,2)+'\n');
