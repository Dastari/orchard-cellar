...
const root=resolve(import.meta.dirname,'../../..'), width=768, height=240, scale=3, baseline=204;
...
for(const [index,name] of ['basalt','cinder','emberglass','ashwood'].entries()) {
  const path=`packages/assets/props/resource_cf_hearth_${name}.sprite.json`, source=await readFile(resolve(root,path),'utf8');
...
await writeFile(resolve(out,'gathering-native-candidates.png'),png);
await writeFile(resolve(out,'gathering-native-candidates.provenance.json'),JSON.stringify({draft:true,scale,baseline,
  order:'basalt, cinder ore, emberglass, ashwood',assets:manifest,imageSha256:createHash('sha256').update(png).digest('hex'),
  reproduce:'npx tsx packages/tools/src/render-hearth-resources.ts'},null,2)+'\n');
