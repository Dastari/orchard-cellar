 ...
 * Existing authored entries are preserved; run content:export afterwards. */
...
import { HEARTH_FURNITURE_CATALOGUE } from './hearth-furniture-catalogue.js';
...
const itemIds = new Set(items.map(item => item.id)), objectIds = new Set(objects.map(object => object.id)), recipeIds = new Set(recipes.map(recipe => recipe.id));
...
for (const entry of HEARTH_FURNITURE_CATALOGUE) {
  ...
  if (!recipeIds.has(recipeId)) recipes.push({ id: recipeId, kind: 'recipe', schemaVersion: 1,
    output: { item: itemId, count: 1 }, stationRequirement: { objectTag: 'station.workbench' }, unlockHint: { book: planId },
    recipeKind: 'shapeless', inputs: Object.entries(entry.materials).map(([material, count]) => ({ item: `item:${material}`, count })) });
