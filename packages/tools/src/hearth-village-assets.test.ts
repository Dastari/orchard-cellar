...
const entries=[{name:'prop_cf_willow_waterfall',category:'props',x:0,y:0,visual:'flow',count:6},...WILLOWHARBOUR_PLOTS.map(plot=>({name:`building_cf_hearth_${plot.id.replaceAll('-','_')}`,category:'buildings',x:0,y:0,visual:'base',count:1})),
  ...
  {name:'prop_cf_hearth_streetlamp',category:'props',x:32,y:0,visual:'on',count:1},
...
it.each([0,1,2,3])('preserves arch column %i with only baked river blue removed',async column=>{
 const asset=JSON.parse(await readFile(resolve(root,`packages/assets/props/prop_cf_hearth_bridge_arch_${column}.sprite.json`),'utf8')) as AssetSource;
 const source=decodePng(await readFile(resolve(root,asset.sourcePath!)));
 for(let y=0;y<20;y++)for(let x=0;x<16;x++){
  const at=((60+y)*source.width+32+column*16+x)*4,expected=source.rgba.slice(at,at+4);
  if(expected[3]===0||(expected[0]===0&&expected[1]===149&&expected[2]===233))expected.fill(0);
  const c=asset.frames.base![0]![y]![x]!,actual=c==='.'?new Uint8Array(4):hexToRgba(asset.sourcePalette![c]!);
