...
describe('overworld lighting', () => {
  it('defaults to Classic while preserving an explicit Unified opt-in', () => {
  ...
  it('keeps water-surface artwork flat while correcting upright decorations', () => {
    ...
    expect(unifiedDecorationLightReceiver('fisher_dock')).toBe('flat');
  ...
  it('keeps darkness within the 23:00-05:00 night window', () => {
    ...
    expect(ambientAtProgress(dayProgressAtClockTime(22))).toEqual({ r: 255, g: 255, b: 255 });
    expect(ambientAtProgress(dayProgressAtClockTime(22, 30))).toEqual({ r: 166, g: 128, b: 157 });
    expect(ambientAtProgress(dayProgressAtClockTime(23))).toEqual(FULL_MOON_NIGHT_AMBIENT);
    expect(ambientAtProgress(dayProgressAtClockTime(4))).toEqual(FULL_MOON_NIGHT_AMBIENT);
    expect(ambientAtProgress(dayProgressAtClockTime(5))).toEqual({ r: 222, g: 174, b: 126 });
  ...
  it('Systems/Lighting & Seasons: preserves Full Moon and makes New Moon deliberately dark only at night', () => {
    ...
    const preDawn = ambientAtProgress(dayProgressAtClockTime(4, 45), 0, 0);
    expect(preDawn.r).toBeGreaterThan(NEW_MOON_NIGHT_AMBIENT.r);
    expect(preDawn.r).toBeLessThan(222);
    expect(ambientAtProgress(dayProgressAtClockTime(5), 0, 0)).toEqual({ r: 222, g: 174, b: 126 });
  });
