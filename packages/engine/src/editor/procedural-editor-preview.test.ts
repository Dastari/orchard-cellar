...
import { terrainProjectedRowsPerLevel } from '@orchard/engine/terrain';
...
describe("procedural editor preview", () => {
  ...
  it("renders deterministic aprons while keeping neighbour payload state ungenerated", () => {
    ...
    const eastApron = sampleProceduralTerrainAt(
      generated.seed, generated.generatorVersion, 16, 0,
    );
    const expectedBiome = proceduralEditorSurvivalBiomeFor(eastApron, [
      sampleProceduralTerrainAt(generated.seed, generated.generatorVersion, 16, -1),
      sampleProceduralTerrainAt(generated.seed, generated.generatorVersion, 17, 0),
      sampleProceduralTerrainAt(generated.seed, generated.generatorVersion, 16, 1),
      sampleProceduralTerrainAt(generated.seed, generated.generatorVersion, 15, 0),
    ]);
    // S3 makes plains legitimately reachable here; compare the generated
    // apron to the coordinate-pure semantic sampler instead of reserving a
    // real biome ordinal as an "uninitialized" sentinel.
    expect(terrain.biomes[eastApronIndex]).toBe(SURVIVAL_BIOMES.indexOf(expectedBiome));
    expect(proceduralEditorChunkGenerated(generated, 1, 0)).toBe(false);
  });
