...
export function proceduralEditorChunkPreviewColors(
  ...
  const overview = sampleProceduralTerrainOverview({
    seed: preview.seed,
    generatorVersion: preview.generatorVersion,
    minTileX: preview.minChunkX * PROCEDURAL_WORLD_CHUNK_TILES
      + Math.floor(PROCEDURAL_WORLD_CHUNK_TILES / 2),
    minTileY: preview.minChunkY * PROCEDURAL_WORLD_CHUNK_TILES
      + Math.floor(PROCEDURAL_WORLD_CHUNK_TILES / 2),
    columns: preview.chunkColumns,
    rows: preview.chunkRows,
    stepTiles: PROCEDURAL_WORLD_CHUNK_TILES,
  });
  for (let offsetY = 0; offsetY < preview.chunkRows; offsetY += 1) {
    for (let offsetX = 0; offsetX < preview.chunkColumns; offsetX += 1) {
      ...
      const sample = overview.samples[offsetY * preview.chunkColumns + offsetX]!;
    ...
    layers: [
      {
        ...
        family: cliffFamilyForProceduralFamily(cell.cliffFamily),
      },
    ],
  };
}
