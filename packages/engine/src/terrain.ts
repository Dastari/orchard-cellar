...
export function terrainForSpace(
  ...
  const terrainKey = `${space.spaceId}:${space.generator}:${space.sizeTiles}:${generatorRevision}:${seed}:${version}:${rogueKey}`;
  const cachedTerrain = terrainCache.get(terrainKey);
  if (cachedTerrain !== undefined) return cachedTerrain;
  const classificationKey = `${space.spaceId}:${space.generator}:${space.sizeTiles}:${generatorRevision}:${seed}:${rogueKey}`;
