...
/** Minimum logical HUD canvas retained at every integer UI scale. The height
 * prevents phone-landscape HUD stacks from selecting 2x and overlapping. */
export const MIN_UI_LOGICAL_WIDTH = 323;
export const MIN_UI_LOGICAL_HEIGHT = VIRTUAL_HEIGHT;
...
export function fittedUiScale(desired: UiScale, canvasWidth: number, canvasHeight: number): UiScale {
  const widthLimit = Math.max(1, Math.floor(canvasWidth / MIN_UI_LOGICAL_WIDTH));
  const heightLimit = Math.max(1, Math.floor(canvasHeight / MIN_UI_LOGICAL_HEIGHT));
