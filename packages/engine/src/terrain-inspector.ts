...
export interface TerrainInspectionVisualOptions {
  readonly layerVisibility?: readonly boolean[];
  readonly selectedLayerIndex?: number | null;
}
...
export function drawTerrainInspectionVisuals(
  ...
  x: number,
  y: number,
  options: TerrainInspectionVisualOptions = {},
): TerrainInspectionVisualLayout {
  const layout = terrainInspectionVisualLayout(inspection);
  const thumbnailRects = terrainInspectionThumbnailRects(inspection, x, y);
  ...
  for (let index = 0; index < inspection.layers.length; index += 1) {
    if (options.layerVisibility?.[index] === false) continue;
    drawLayerPreview(
      context,
      art,
      terrain,
  ...
  for (let index = 0; index < inspection.layers.length; index += 1) {
    const rect = thumbnailRects[index]!;
    const visible = options.layerVisibility?.[index] !== false;
    drawTransparencyGrid(context, rect.x, rect.y, rect.width);
    context.save();
    if (!visible) context.globalAlpha = 0.25;
    drawLayerPreview(
      context,
      art,
      terrain,
    ...
    context.fillRect(rect.x, rect.y, 10, 8);
    context.fillStyle = '#f6f0d8';
    context.font = '7px monospace';
    context.textBaseline = 'top';
    context.fillText(String(index + 1), rect.x + 1, rect.y);
    const toggle = rect.visibilityToggle;
    context.fillStyle = visible ? '#315f36ee' : '#71372fee';
    context.fillRect(toggle.x, toggle.y, toggle.width, toggle.height);
    context.fillStyle = '#f6f0d8';
    context.fillText(visible ? 'O' : 'X', toggle.x + 2, toggle.y + 1);
    context.strokeStyle = options.selectedLayerIndex === index ? '#f5df75' : '#887f72';
    context.lineWidth = options.selectedLayerIndex === index ? 2 : 1;
