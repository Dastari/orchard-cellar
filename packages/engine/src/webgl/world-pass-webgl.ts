...
export interface WebGLSourceOptions {
  ...
  readonly ground?: { readonly field: WebGLGroundField; readonly worldX: number; readonly worldY: number; readonly basis?:GroundSpriteBasis };
...
export class WebGLWorldPassBackend implements WorldPassBackend {
  ...
  private image(image:CanvasImageSource,rect:readonly number[],state:CanvasState):void {
    ...
    const light=field ? groundLightTextureCoordinates(options.ground!,rect[2]!,rect[3]!):[0,0,0,0];
...
/** One light UV per native corner; rotations/reflections are not UV rectangles. */
export function groundLightTextureCoordinates(ground:NonNullable<WebGLSourceOptions['ground']>,width:number,height:number):number[]{
  const field=ground.field,basis=ground.basis??{a:1,b:0,c:0,d:1};
  return [0,1,2,3].flatMap(corner=>{
    const x=(corner&1)?width:0,y=(corner&2)?height:0;
    return [(ground.worldX+basis.a*x+basis.c*y-field.left)/(field.width*field.step),
      (ground.worldY+basis.b*x+basis.d*y-field.top)/(field.height*field.step)];
  });
}
