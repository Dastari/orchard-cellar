...
import { bakedShadowSelection, loadGeneratedAsset, type LoadedAsset } from '@orchard/ui';
...
const MAP_EDITOR_ASSET_NAMES = {
  ...
  poiFlowersPink: 'prop_cf_flowers_pink',
  poiFlowersGold: 'prop_cf_flowers_gold',
  poiStump: 'prop_cf_poi_stump',
  poiFallenLog: 'prop_cf_poi_fallen_log',
  poiRockSmall: 'prop_cf_poi_rock_small',
  campTent: 'prop_cf_camp_tent',
  homesteadTentMarker: 'prop_cf_homestead_tent_marker',
  homesteadTentLarge: 'prop_cf_homestead_tent_large',
  campfire: 'prop_cf_campfire',
  campRoundStool: 'prop_cf_camp_round_stool',
  campBench: 'prop_cf_camp_bench',
  campStumpSeat: 'prop_cf_camp_stump_seat',
  campChair: 'prop_cf_camp_chair',
  campFishingRod: 'prop_cf_camp_fishing_rod',
  campPond: 'prop_cf_pond',
  farmHouse: 'building_cf_farmhouse',
  farmHayBale: 'prop_cf_farm_hay_bale',
  farmHayStack: 'prop_cf_farm_hay_stack',
  farmPottedFlowers: 'prop_cf_farm_potted_flowers',
  farmGrave: 'prop_farmer_jane_grave',
  fishermanHut: 'building_cf_fisherman_hut',
  fishingDock: 'prop_cf_fishing_dock',
  trapdoor: 'prop_cf_trapdoor',
  cellarLadder: 'prop_cf_cellar_ladder',
  interiorDoor: 'prop_cf_interior_door',
  interiorBed: 'prop_cf_interior_bed',
  interiorBookshelf: 'prop_cf_interior_bookshelf',
  interiorTable: 'prop_cf_interior_table',
  caveSupport: 'prop_cf_cave_support',
...
export async function loadMapEditorArt(ui: PixelUi, uiSkin: UiSkin): Promise<OverworldArt> {
...
export async function loadOverworldArt(): Promise<OverworldArt> {
  const [
    ...
    loadGeneratedAsset("prop_farmer_jane_grave", "summer"),
    loadGeneratedAsset("building_cf_fisherman_hut", "summer"),
    loadGeneratedAsset("prop_cf_fishing_dock", "summer"),
...
export function merchantWorldBounds(
  ...
  const vertical = facing === 'up' || facing === 'upLeft' || facing === 'upRight'
    ...
    : facing === 'down' || facing === 'downLeft' || facing === 'downRight'
      ? 'down'
      : 'right';
  const animation = npcKind === 'fisherman_fin' && !moving
    ? activity === 'fish' || activity === 'fish_cast' || activity === 'fish_wait'
      ? `cast_${vertical}`
      : activity === 'fish_reel'
        ? `reel_${vertical}`
  ...
  const visualFrame = npcKind === 'fisherman_fin'
    && (activity === 'fish' || activity === 'fish_wait')
    ? 1
    : animationFrame;
  const flip =
  ...
  return anchoredVisualBounds(
    npcKind === 'fisherman_fin' ? art.fishermanFinNpc
  ...
  const asset =
    npc.npcKind === "merchant" || npc.npcKind === 'farmer_bob' || npc.npcKind === 'fisherman_fin'
      ...
      : (rogueEnemy ?? variants?.[npc.variant % Math.max(1, variants.length)] ??
        art.missingItem);
  const preferred = npc.npcKind === "merchant" || npc.npcKind === 'farmer_bob'
...
export function wildlifeAnimationName(
  ...
  // Hay eating is a durable authority phase, but it deliberately reuses each
  // species' authored grazing/foraging animation on the client.
...
export function wildlifeShadowBody(art: OverworldArt, species: WildlifeSpecies, variant: number,
  ...
  const visual = resolveWildlifeVisual(art, species, variant, 'idle', facing, false, 0, inWater);
  const airborne = species === 'bee' || species === 'butterfly'
    || (species === 'vulture' && moving && activity !== 'rest' && activity !== 'sleep');
...
export function actionVisualForDirection(
  ...
  const bodyOnly = actionKind === 'jump' || authoredAsset === null;
