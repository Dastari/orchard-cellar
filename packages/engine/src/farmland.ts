...
function drawSoilFrame(
  ...
  const frame = worldAssetFrameSource(context, asset, selected)!;
  context.drawImage(
    frame.image,
    frame.x,
    frame.y,
...
export function drawInteractionTileReticle(
  ...
  context.drawImage(
    ...
    Math.round((tileY * 16 - terrainProjectionPixels - cameraY) * scale - destinationInset),
    destinationSize,
    destinationSize,
  );
}
