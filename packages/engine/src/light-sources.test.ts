...
describe('placed crafting light emitters', () => {
  ...
  it('projects authored light definitions through the shared point-light shape', () => {
    const row = { id: 42n, kind: 'oil_lamp', tileX: 2, tileY: 3, lit: false };
    expect(placeablePointLight(row, 10n, {
      enabled: true, color: [255, 196, 120], radiusTiles: 4,
      profile: 'steady', offsetY: -6,
    })).toEqual({
      worldX: 40, worldY: 58, receiverDirectionWorldY: 64,
      radiusTiles: 4, color: { r: 255, g: 196, b: 120 },
      strengthPerMille: 1000, profile: 'steady',
    });
    expect(placeablePointLight(row, 10n, {
      enabled: false, color: [255, 196, 120], radiusTiles: 4, profile: 'steady',
    })).toBeNull();
  });
