...
function fixture() {
  ...
  const blobs = [0, 1].map(cx => encodeWorldChunk({ schema: 1, spaceId: 1, cx, cy: 0, assetRevision: 'a',
    arrays: Object.fromEntries(Object.entries(arrays).map(([name, value]) => [name, sliceWorldChunkChannel(value, width, height, cx, 0)])),
