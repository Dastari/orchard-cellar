...
import { atlasFrames, selectAtlasFrame } from '@orchard/ui';
...
export interface WindTreeSource {
  ...
  readonly leafProfile?: 'oak' | 'birch' | 'spruce';
...
export function treeSwayOffset(
  ...
  // Automatic weather peaks at 0.34–0.42. Whole-world-pixel rounding made
  // every one of those breezes completely still, despite emitting leaves.
  // Ease in a readable crown bend, keeping the same maximum storm amplitude;
  // the renderer scales these fractional offsets before rasterizing the tree.
  const strength = Math.min(1, (weather.wind - 0.3) / 0.7);
  const amplitude = Math.sqrt(strength) * 1.8;
  return [
    weather.windDirectionX * wave * amplitude,
    weather.windDirectionY * wave * amplitude * 0.75,
  ];
}
...
function drawFrame(
  ...
  const source = worldAssetFrameSource(context, asset, frame);
  ...
  if (flipX) {
    ...
    context.scale(-1, 1);
    context.drawImage(source.image, source.x, source.y, source.width, source.height, 0, drawY, width, height);
    context.restore();
  } else {
    context.drawImage(source.image, source.x, source.y, source.width, source.height, drawX, drawY, width, height);
  }
  return true;
}
...
export class WeatherEffects {
  ...
  private drawTreeLeaves(
    ...
    for (const tree of trees) {
      ...
      for (let offset = 0; offset < generations; offset += 1) {
        ...
        const canopyWidth = tree.canopyWidth ?? 34;
        ...
        if (drawFrame(
          context,
          this.leafAsset(tree.leafProfile ?? 'oak'),
          'base',
          0,
          (worldX - cameraX) * scale,
          (worldY - cameraY) * scale,
  ...
  private leafAsset(profile: 'oak' | 'birch' | 'spruce'): LoadedAsset {
    if (profile === 'birch') return this.birchLeaf;
    if (profile === 'spruce') return this.spruceLeaf;
    return this.oakLeaf;
  }
}
