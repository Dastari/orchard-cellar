...
/** Classic is the default while seasonal rendering is being optimized. */
...
const NIGHTFALL_START = dayProgressAtClockTime(22, 30);
const FULL_NIGHT_START = dayProgressAtClockTime(23);
const PRE_DAWN_START = dayProgressAtClockTime(4, 30);
const NIGHT_END = dayProgressAtClockTime(5);
...
const AMBIENT_KEYFRAMES: readonly AmbientKeyframe[] = [
  ...
  { progress: dayProgressAtClockTime(22), color: { r: 255, g: 255, b: 255 } },
  { progress: NIGHTFALL_START, color: { r: 166, g: 128, b: 157 } },
  { progress: FULL_NIGHT_START, color: { r: 89, g: 89, b: 105 } },
  { progress: PRE_DAWN_START, color: { r: 89, g: 89, b: 105 } },
  { progress: NIGHT_END, color: { r: 222, g: 174, b: 126 } },
...
export function lunarNightWeightAtProgress(dayProgress: number): number {
  ...
  if (progress < NIGHT_END) {
    return 1 - (progress - PRE_DAWN_START) / (NIGHT_END - PRE_DAWN_START);
  }
  return 0;
}
...
export class TileLightmap {
  private surfaces: { canvas: HTMLCanvasElement; context: CanvasRenderingContext2D; haloCanvas: HTMLCanvasElement; haloContext: CanvasRenderingContext2D } | null = null;
  private floodValue: QuantizedLightFlood | null = null;
  private get flood(): QuantizedLightFlood { return this.floodValue ??= new QuantizedLightFlood(); }
  private ensureSurfaces() {
    if (this.surfaces !== null) return this.surfaces;
    const canvas = document.createElement('canvas');
    const haloCanvas = document.createElement('canvas');
    const context = canvas.getContext('2d');
    const haloContext = haloCanvas.getContext('2d');
    if (context === null || haloContext === null) throw new Error('Lightmap Canvas 2D unavailable');
    return this.surfaces = { canvas, context, haloCanvas, haloContext };
  }
  private get canvas(): HTMLCanvasElement { return this.ensureSurfaces().canvas; }
  private get context(): CanvasRenderingContext2D { return this.ensureSurfaces().context; }
  private get haloCanvas(): HTMLCanvasElement { return this.ensureSurfaces().haloCanvas; }
  private get haloContext(): CanvasRenderingContext2D { return this.ensureSurfaces().haloContext; }
  ...
  private readonly receiverPixelsByElevation = new Map<number, Uint8ClampedArray>();
  private readonly receiverFaceRgbByElevation = new Map<number, Uint8ClampedArray>();
  private retainedReceiverFields = false;
  ...
  reset(): void {
    ...
    this.surfaces = null; this.floodValue = null;
    this.image = null; this.haloImage = null;
    this.pixels = new Uint8ClampedArray(0); this.lightPixels = new Uint8ClampedArray(0); this.haloPixels = new Uint8ClampedArray(0);
    this.southFacePixelsByElevation.clear(); this.occlusionLayers.clear();
    this.receiverPixelsByElevation.clear(); this.receiverFaceRgbByElevation.clear(); this.retainedReceiverFields = false;
    this.lastOcclusionMap = null; this.haloVisible = false;
    this.lastMinTileX = this.lastMinTileY = this.lastLightSignature = this.lastAmbientSignature = Number.NaN;
    this.floodTexelsVisitedValue = this.floodMsValue = this.fieldRebuildsValue = 0;
    this.occlusionRebuildsValue = this.occlusionCacheHitsValue = 0;
    this.boundsResizeMsValue = this.rasterizeMsValue = this.mergeMsValue = this.uploadMsValue = this.receiverMsValue = this.compositeMsValue = 0;
  ...
  prepare(
    ...
    const rebuild = minTileX !== this.lastMinTileX || minTileY !== this.lastMinTileY
      ...
      || lightingModel !== this.lastLightingModel || retainReceiverFields !== this.retainedReceiverFields;
    ...
    if (rebuild) {
      ...
      if (!retainReceiverFields) { this.receiverPixelsByElevation.clear(); this.receiverFaceRgbByElevation.clear(); }
      else {
        for (const pixels of this.receiverPixelsByElevation.values()) pixels.fill(0);
        for (const pixels of this.receiverFaceRgbByElevation.values()) pixels.fill(0);
      }
      ...
      if (retainReceiverFields && lightsByElevation.size > 8) throw new Error('receiver_light_level_budget_exceeded');
      for (const layers of [this.receiverPixelsByElevation, this.receiverFaceRgbByElevation]) {
        for (const level of layers.keys()) if (!lightsByElevation.has(level)) layers.delete(level);
      }
      ...
      for (const [elevation, elevationLights] of lightsByElevation) {
        const receiverPixels = retainReceiverFields ? this.receiverLayer(this.receiverPixelsByElevation, elevation, width * height * 4) : this.lightPixels;
        const receiverFaceRgb = retainReceiverFields ? this.receiverLayer(this.receiverFaceRgbByElevation, elevation, width * height * 4) : null;
        ...
        if (retainReceiverFields) for (let offset = 0; offset < this.lightPixels.length; offset += 4) {
          this.lightPixels[offset] = Math.max(this.lightPixels[offset]!, receiverPixels[offset]!);
          this.lightPixels[offset + 1] = Math.max(this.lightPixels[offset + 1]!, receiverPixels[offset + 1]!);
          this.lightPixels[offset + 2] = Math.max(this.lightPixels[offset + 2]!, receiverPixels[offset + 2]!);
        }
      ...
      this.retainedReceiverFields = retainReceiverFields;
  ...
  private receiverLayer(layers: Map<number, Uint8ClampedArray>, elevation: number, size: number): Uint8ClampedArray {
    let pixels = layers.get(elevation);
    if (pixels === undefined || pixels.length !== size) { pixels = new Uint8ClampedArray(size); layers.set(elevation, pixels); }
    return pixels;
  }
  ...
  private resize(width: number, height: number): void {
    ...
    this.receiverPixelsByElevation.clear(); this.receiverFaceRgbByElevation.clear();
