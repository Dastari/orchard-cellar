...
export const PLACEABLE_LIGHT_EMITTERS: Readonly<Record<string, LightEmitterDefinition>> = {
  ...
  standing_torch: {
    color: TORCH_LIGHT, radiusTiles: TORCH_LIGHT_RADIUS_TILES, offsetY: -15, profile: 'flame', facingSeed: false,
  },
};
...
export function placeablePointLight(
  ...
  authored?: ResolvedObjectLight,
): PointLight | null {
  if (authored?.enabled === false || (authored === undefined && placeable.lit === false)) return null;
  const emitter = authored === undefined ? PLACEABLE_LIGHT_EMITTERS[placeable.kind] : {
    color: { r: authored.color[0], g: authored.color[1], b: authored.color[2] },
    radiusTiles: authored.radiusTiles,
    offsetY: authored.offsetY ?? 0,
    profile: authored.profile === 'flicker' ? 'flame' as const : 'steady' as const,
    facingSeed: false,
  };
