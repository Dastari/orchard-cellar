...
describe('OIDC browser session', () => {
  ...
  it('adopts only valid same-issuer peer sessions and never copies refresh credentials', () => {
    const session: OidcSession = {
      idToken: jwt({ sub: 'user_1' }), refreshToken: 'tab-private', expiresAt: 200_000,
      subject: 'user_1', displayName: 'Farmer', emailVerified: true,
      issuer: 'https://auth.orchard.dastari.net/realms/orchard',
    };
    expect(parseOidcPeerSession(session, 100_000)).toEqual({ ...session, refreshToken: null });
    expect(parseOidcPeerSession({ ...session, issuer: 'https://wrong.example' }, 100_000)).toBeNull();
    expect(parseOidcPeerSession({ ...session, expiresAt: 120_000 }, 100_000)).toBeNull();
  });
