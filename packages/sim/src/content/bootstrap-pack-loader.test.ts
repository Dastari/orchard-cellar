...
describe('bootstrap content pack loader', () => {
  it('loads the complete committed pack deterministically through one parser boundary', () => {
    ...
    expect(definitions).toHaveLength(947);
    expect(new Set(definitions.map(({ id }) => id)).size).toBe(947);
    expect(contentDefinitionsHash(definitions).slice(0, 8)).toBe('3cc75c4a');
