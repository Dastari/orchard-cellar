...
export type HearthInteriorContentKind = 'i' | 'g' | 'c' | 'f' | 's' | 'u' | 'h' | 'o' | 'b' | 'p';
...
export interface HearthInteriorFloorRegion {
  readonly bounds: readonly [number, number, number, number];
  readonly style: 'townhouse' | 'stone' | 'soil';
}
...
export interface SpaceContentDefinition extends WorldDefinitionBase<'space', SpaceContentDefinitionId> {
  ...
  readonly hearthInteriorFloors?: readonly HearthInteriorFloorRegion[];
  readonly hearthInteriorWindows?: readonly (readonly [number,number])[];
...
function parseHearthInteriorFloors(value: unknown): readonly HearthInteriorFloorRegion[] {
  return array(value, '$.hearthInteriorFloors').map((entry, index) => {
    const path = `$.hearthInteriorFloors[${index}]`, region = object(entry, path);
    const bounds = compactTuple(region.bounds, `${path}.bounds`, [4])
      .map((coordinate, axis) => integer(coordinate, `${path}.bounds[${axis}]`, 0)) as [number, number, number, number];
    const style = string(region.style, `${path}.style`);
    if (style !== 'townhouse' && style !== 'stone' && style !== 'soil')
      throw new ContentParseError('invalid_type', `${path}.style`, 'unknown interior floor style');
    return { bounds, style };
  });
}
...
function parseHearthInterior(value: unknown): HearthInteriorContentDefinition {
  ...
  if (!['i', 'g', 'c', 'f', 's', 'u', 'h', 'o', 'b', 'p'].includes(kind)) {
