...
export type FrameEntityContainer = 'chest' | 'placeable';
...
export interface FrameButtonDefinition {
  ...
  readonly onInvoke?: { readonly claimProcessJob: 'collect' | 'cancel' };
...
function parseOnInvoke(value: unknown, path: string): NonNullable<FrameButtonDefinition['onInvoke']> {
  const source = record(value, path);
  if (Object.keys(source).length !== 1 || !Object.prototype.hasOwnProperty.call(source, 'claimProcessJob')
    || (source.claimProcessJob !== 'collect' && source.claimProcessJob !== 'cancel')) {
    throw new Error(`${path}: expected exactly one bounded collect or cancel claimProcessJob callback`);
  }
...
export function parseFrameDefinition(json: string | unknown): FrameContentDefinition {
  ...
  if (entityContainer !== undefined && entityContainer !== 'chest' && entityContainer !== 'placeable') {
