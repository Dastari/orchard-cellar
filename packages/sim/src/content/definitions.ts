...
import type { BalanceContentDefinition, BalanceDefinitionId, BalanceUnit } from './balance-definition.js';
import {
...
function integer(value: unknown, path: string, minimum = 0): number {
  if (!Number.isSafeInteger(value) || (value as number) < minimum) {
    fail('invalid_type', path, `expected a safe integer >= ${minimum}`);
  }
...
export function parseItemDefinition(json: string | unknown): ItemContentDefinition {
  ...
  const quality = stringValue(source.quality, '$.quality');
...
export function parseProcessDefinition(json: string | unknown): ProcessContentDefinition {
  ...
  return Object.freeze({
    ...
    ...(experience === undefined ? {} : { experience: {
      ...
      amount: integer(experience.amount, '$.experience.amount'),
    } }),
