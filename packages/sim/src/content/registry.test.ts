...
import { runtimeVigourDefinition } from './runtime.js';
...
describe('Phase-0 content registry', () => {
  it('projects the bootstrap pack back to every migrated compiled table', () => {
    ...
    for (const [kind, balance] of Object.entries(TOOL_DURABILITY_BALANCE)) {
      expect(registry.items.get(`item:${kind}`)?.durability).toEqual({
        max: balance.maximum,
        repairMaterial: `item:${balance.repairItemKind}`,
        repairCost: ANVIL_REPAIR_COST_BRONZE,
      });
    }
