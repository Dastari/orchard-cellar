...
export function compiledProjection(
  ...
  return deepFreeze({
    ...
    merchantOffers,
    crops: crops.map(compiledCropProjection),
    creatures: Object.fromEntries(creatures.map((definition) => [definition.species, compiledCreatureProjection(definition)])),
    ...
    effects: Object.fromEntries(effects.map((definition) => [slug(definition.id), {
    ...
    statistics: Object.fromEntries(statistics.map((definition) => [slug(definition.id), compiledStatisticProjection(definition)])),
    upgrades: Object.fromEntries(upgrades.map((definition) => [slug(definition.id), compiledUpgradeProjection(definition)])),
  });
}
