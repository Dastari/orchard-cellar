...
export { contentDefinitionsHash, contentDefinitionRowsHash } from './payload-hash.js';
...
function invalidBuildReport(
  ...
  const validation = validateContentDefinitions(parsed);
...
export function buildContentRegistry(rows: readonly ContentDefinitionRow[]): BuildContentRegistryResult {
  ...
  return Object.freeze({ registry, report: invalidBuildReport(sorted, parsingErrors) });
}
