...
export interface ItemContainerContentResolver {
 ...
 * intentionally resolve to null/false so stale or unknown rows fail closed. */
export function itemContainerContentResolver(
  registry: Pick<ContentRegistry, 'items'>,
): ItemContainerContentResolver {
  return Object.freeze({
    maxStackFor: (itemKind: string) => registry.items.get(`item:${itemKind}`)?.maxStack ?? null,
    hasTag: (itemKind: string, tag: string) => registry.items.get(`item:${itemKind}`)?.tags.includes(tag) === true,
    displayNameFor: (itemKind: string) => registry.items.get(`item:${itemKind}`)?.displayName ?? null,
  });
}
...
export function fillCraftingRecipeFromInventory(
  ...
  const gridWidth = Math.sqrt(crafting.capacity);
  if (!Number.isInteger(gridWidth)) return failure('index_out_of_capacity');

  const desired = Array.from({ length: crafting.capacity }, () => null as string | null);
  if (recipe.kind === 'shaped') {
    const normalized = normalizeShapedRecipe(recipe);
    if (normalized === null || normalized.width > gridWidth || normalized.height > gridWidth) {
      return failure('recipe_not_found');
    }
    for (let y = 0; y < normalized.height; y += 1) {
      for (let x = 0; x < normalized.width; x += 1) {
        desired[y * gridWidth + x] = normalized.pattern[y * normalized.width + x] ?? null;
      }
    }
  } else {
    let target = 0;
    for (const [itemKind, quantity] of Object.entries(recipe.inputs)) {
      for (let count = 0; count < quantity; count += 1) {
        if (target >= desired.length) return failure('recipe_not_found');
        desired[target] = itemKind;
        target += 1;
      }
    }
  }
  ...
  for (let index = 0; index < desired.length; index += 1) {
    ...
    if (current !== null && (expected === null || current.itemKind !== expected || current.quantity <= 0)) {
  ...
  for (let targetIndex = 0; targetIndex < desired.length; targetIndex += 1) {
    const itemKind = desired[targetIndex];
    if (itemKind === null || next.crafting?.slots[targetIndex]?.itemKind === itemKind) continue;
