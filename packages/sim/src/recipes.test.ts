...
import { recipeBookDefinition, recipeBookPatternIds, recipeIngredientStacks, recipeMatches, RECIPES, normalizeShapedRecipe } from './recipes.js';
...
describe('06§12 crafting recipe goldens', () => {
  it('uses Minecraft\'s five-plank U pattern for boats', () => {
    expect(RECIPES.boat!).toMatchObject({
      station: 'workbench',
      pattern: [['plank', null, 'plank'], ['plank', 'plank', 'plank']],
      output: { itemKind: 'boat', quantity: 1 },
    });
    expect(recipeIngredientStacks(RECIPES.boat!)).toEqual([{ itemKind: 'plank', quantity: 5 }]);
  });
  ...
  it('keeps Marlow\'s consumable starter book useful without revealing specialty recipes', () => {
    ...
    expect(book?.recipeIds).toContain('watch');
    expect(book?.recipeIds).toContain('string');
    expect(book?.recipeIds).toContain('backpack');
    ...
    const fishing = recipeBookDefinition('fishing_handbook');
    expect(fishing === null ? [] : recipeBookPatternIds(fishing)).toEqual(['fishing_rod', 'cook_fish']);
