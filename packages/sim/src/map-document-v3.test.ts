...
describe('MapDocumentV3', () => {
  it('round-trips registry-authored landmark group ids outside the original island groups', () => {
    const decorations = generateSurvivalLandmarkDecorations([{
      ...
      decorations: [{
        kind: 'point', decorationKind: 'camp_flowers', tileX: 3, tileY: 3, role: 'soil.watered',
      }],
    }]);
    ...
    expect(parsed.landmarks[0]?.role).toBe('soil.watered');
  ...
  it('hydrates semantic roles onto an existing live landmark document without trusting its old identities', () => {
    const authored = [{
      id: 'renamed_water_garden', label: 'Renamed Water Garden', runtimeIdBase: '8000000000',
      bounds: { minimumTileX: 2, maximumTileX: 4, minimumTileY: 2, maximumTileY: 4 },
      decorations: [{
        kind: 'point' as const,
        ...
        tileX: 3,
        tileY: 3,
        role: 'soil.watered' as const,
      }],
    }];
    const document = createLiveIslandMapDocument({ landmarks: authored });
    const legacyJson = JSON.parse(serializeMapDocumentV3(document)) as {
      landmarks: Array<Record<string, unknown>>;
    };
    delete legacyJson.landmarks[0]!['role'];
    legacyJson.landmarks[0]!['groupId'] = 'old_landmark_identity';
    legacyJson.landmarks[0]!['kind'] = 'retired_crop_canvas';
    const parsed = parseMapDocumentV3(JSON.stringify(legacyJson), authored);
    expect(parsed.landmarks[0]).toMatchObject({
      groupId: 'old_landmark_identity',
      kind: 'retired_crop_canvas',
      role: 'soil.watered',
    });
    const unwatered = [{
      ...authored[0]!,
    ...
    expect(parseMapDocumentV3(serializeMapDocumentV3(parsed), unwatered).landmarks[0])
