...
import {
  cellFamilyAt,
  compiledElevationRange,
  compiledMapElevationAt,
  compiledMapTerrainPlaneCollisionBytes,
  rampRoleAt,
  type CompiledMapDocument,
} from './terrain-plane-collision.js';
// Generator-free leaf (static-world S6a); re-exported so existing imports keep working.
export {
  cellFamilyAt,
  compiledMapElevationAt,
  compiledMapTerrainPlaneCollisionBytes,
...
export function compileMapDocument(document: MapDocumentV2, tilesets?: RuntimeTilesetResolver): CompiledMapDocument {
  ...
  // Prime the shared elevation-range cache (compiledElevationRange stores it).
  compiledElevationRange(compiled);
  return compiled;
}
