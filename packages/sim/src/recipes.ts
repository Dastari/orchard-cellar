...
/** Grid targets shared by authoritative autofill and its visible ingredient ghosts. */
export function recipeGridStacks(recipe: RecipeDefinition, capacity: number,
  maximumFor: (kind: string) => number | null): readonly (ItemStack | null)[] | null {
  const width = Math.sqrt(capacity);
  if (!Number.isInteger(width) || capacity < 1) return null;
  const targets: (ItemStack | null)[] = Array.from({ length: capacity }, () => null);
  if (recipe.kind === 'shaped') {
    const normalized = normalizeShapedRecipe(recipe);
    if (!normalized || normalized.width > width || normalized.height > width) return null;
    for (let y = 0; y < normalized.height; y++) for (let x = 0; x < normalized.width; x++) {
      const itemKind = normalized.pattern[y * normalized.width + x];
      if (itemKind) targets[y * width + x] = { itemKind, quantity: 1 };
    }
    return targets;
  }
  const compact = Object.values(recipe.inputs).reduce((sum, count) => sum + count, 0) > capacity;
  let index = 0;
  for (const [itemKind, count] of Object.entries(recipe.inputs)) {
    const maximum = compact ? maximumFor(itemKind) : 1;
    if (!maximum || maximum < 1) return null;
    for (let remaining = count; remaining > 0; remaining -= maximum) {
      if (index >= capacity) return null;
      targets[index++] = { itemKind, quantity: Math.min(maximum, remaining) };
    }
  }
  return targets;
}
