...
export function createSurvivalAuthoredLandmarkInstances(): readonly MapLandmarkInstance[] {
  return survivalAuthoredLandmarkDecorations().map((decoration) => ({
...
export function migrateMapDocumentV2(document: MapDocumentV2): MapDocumentV3 {
  return normalizeMapDocumentV3({
    ...
    prefabs: [],
    objects: [],
    landmarks: document.id === LIVE_ISLAND_MAP_ID ? createSurvivalAuthoredLandmarkInstances() : [],
...
function parseLandmarkInstance(value: unknown, width: number, height: number): MapLandmarkInstance | null {
  ...
  if (candidate === null
    ...
    || !SURVIVAL_DECORATION_KINDS.includes(candidate['kind'] as SurvivalDecorationKind)
    || !integerInRange(candidate['tileX'], 0, width - 1)
    || !integerInRange(candidate['tileY'], 0, height - 1)
    || !integerInRange(candidate['elevation'], -32, 32)
    || !MAP_OBJECT_LAYER_IDS.includes(candidate['layer'] as MapObjectLayer)
  ...
  return {
    ...
    groupId: candidate['groupId'] as SurvivalAuthoredLandmarkGroupId,
    groupLabel: candidate['groupLabel'],
    kind: candidate['kind'] as SurvivalDecorationKind,
...
export function parseMapDocumentV3(source: string): MapDocumentV3 {
  let value: unknown;
  ...
  if (candidate['schemaVersion'] === 2) return migrateMapDocumentV2(parseMapDocument(source));
  ...
  const landmarkSource = candidate['landmarks'] === undefined && terrain.id === LIVE_ISLAND_MAP_ID
    ? createSurvivalAuthoredLandmarkInstances().map((landmark) => ({
        ...landmark,
  ...
  if (layers.some((layer) => layer === null) || objects.some((object) => object === null)
    || landmarks.some((landmark) => landmark === null)) {
  ...
  return normalizeMapDocumentV3({
    ...
    landmarks: landmarks as MapLandmarkInstance[],
    generatedSuppressions: (candidate['generatedSuppressions'] as string[]).filter((id) => (
      !landmarks.some((landmark) => id === `decoration-${landmark!.sourceDecorationId}`)
    )),
  });
}
...
export function mapLandmarkCollisionObstacle(
  ...
  const obstacle = survivalDecorationObstacle(mapLandmarkDecoration(landmark), medium);
