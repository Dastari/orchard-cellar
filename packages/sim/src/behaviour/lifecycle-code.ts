...
export interface ItemOnUsePlayerContext {
  ...
  restoreHunger(amountCenti: number): void;
}
...
function executeItemOnUse(
  ...
  const context: ItemOnUseContext = Object.freeze({
    ...
    player: Object.freeze({
      ...
      restoreHunger: (amountCenti: number) => emit({
        restoreHunger: positiveCount(amountCenti, 'hunger restore'),
      }),
    }),
    item: Object.freeze({
      snapshot: item,
      ...
      repair: () => emit({ repairSelected: true }),
