...
describe('data graph behaviour compiler', () => {
  it('compiles authored food and tea use with legacy effect and full-hunger parity', () => {
    ...
    expect(itemEffects(cookedBeef, 5000)).toEqual({ effects: [
      { consumeSelected: 1 },
      { applyEffect: { effectId: 'hunger', stacks: 4000 } },
    ] });
    ...
    expect(itemEffects(apple, 10_000)).toEqual({ effects: [
      { consumeSelected: 1 },
      { applyEffect: { effectId: 'hunger', stacks: 700 } },
