...
describe('authored frame lifecycle callbacks', () => {
  ...
  it('authors both station collection and anywhere inventory cancellation', () => {
    ...
    for (const [definition, action] of [[cooking, 'collect'], [cooking, 'cancel'], [inventory, 'cancel']] as const) {
      const button = definition.buttons!.find((entry) => entry.onInvoke !== undefined
        && 'claimProcessJob' in entry.onInvoke && entry.onInvoke.claimProcessJob === action)!;
    ...
    expect(inventory.buttons?.some((entry) => entry.onInvoke !== undefined
      && 'claimProcessJob' in entry.onInvoke && entry.onInvoke.claimProcessJob === 'collect')).toBe(false);
    const barrel = content.frames.get('frame:barrel')!;
    const seal = barrel.buttons!.find((entry) => entry.onInvoke !== undefined && 'sealContainer' in entry.onInvoke)!;
    expect(raiseEvent(registry, { type: 'frameAction', actor, frameId: barrel.id,
      actionId: seal.interaction }, snapshot)).toEqual({ effects: [{ sealContainer: true }] });
  });

  it('routes container sealing by authored callback rather than frame or action id', () => {
    const definition = parseFrameDefinition({
      id: 'frame:moon_vat', kind: 'frame', schemaVersion: 1, title: 'MOON VAT', style: 'wood',
      panes: [{ id: 'status', kind: 'label', bind: { state: 'batchLabel' } }],
      buttons: [{ label: 'CLOSE BATCH', interaction: 'bind_moonlight', onInvoke: { sealContainer: true } }],
    });
    const registry = createHandlerRegistry(frameActionHandlerRegistrations([definition]));
    expect(raiseEvent(registry, {
      type: 'frameAction', actor, frameId: definition.id, actionId: 'bind_moonlight',
    }, snapshot)).toEqual({ effects: [{ sealContainer: true }] });
  });

  it('strictly rejects ambiguous or unbounded callback bindings and duplicate action ids', () => {
    const definition = frame();
    for (const onInvoke of [null, {}, { claimProcessJob: 'start' }, { sealContainer: false },
