...
for (const recipe of recipes) {
  ...
  it('accepts only the bounded hunger and repair capability paths', () => {
    const fixture = sourceFixture(`context.player.restoreHunger(700);
context.item.repair();`);
    expect(() => validateLifecycleSourceBundleAst(fixture.bundle)).not.toThrow();
    const artifacts = compileLifecycleBundle(fixture.bundle, fixture.bundleSha256);
    expect(artifacts.serverTypeScript).toContain('context.player.restoreHunger(700);');
    expect(artifacts.serverTypeScript).toContain('context.item.repair();');

    const arbitrary = sourceFixture('context.player.repair();');
    expect(() => validateLifecycleSourceBundleAst(arbitrary.bundle))
      .toThrow('outside the lifecycle capability API: context.player.repair');
  });
...
context.item.damage();`, ['secondary', 'worldItemUse', 'useWith', 'useAt', 'aimedUse', 'place']);
  ...
  it('executes baseline recipe-book, food, and tea callbacks as bounded effects', () => {
    ...
    expect(run('item:apple.on_use', {
      ...
      { consumeSelected: 1 }, { restoreHunger: 700 },
  ...
  it('executes exact authored placement and repair callbacks with legacy-parity effects', () => {
    ...
    expect(run('item:axe.world_tool.useWith', useWith, {
      ...
      { chargeBronze: 5 },
      { repairSelected: true },
      { statistic: { kind: 'tools_repaired', subject: 'axe' } },
    ] });
