...
describe('selected-item lifecycle capability audit', () => {
  ...
  it('derives every active item category from its declared runtime owner', () => {
    ...
    for (const rule of manifest.derivedCapabilityRules) {
      ...
      for (const itemId of expected ?? []) {
        if (actionOwners.furnitureTransactions.itemIds.includes(itemId) && rule.id === 'placeable-placement') {
          expect(metadataTriggers(itemId).size, `${itemId} must not expose unsafe generic placement`).toBe(0);
          for (const reducer of actionOwners.furnitureTransactions.reducers) {
            expect(worldSource).toContain(`export const ${reducer} = spacetimedb.reducer`);
            expect(connectionSource).toContain(`${reducer}(`);
            expect(existsSync(new URL(`../../world-bindings/src/${camelToSnake(reducer)}_reducer.ts`, import.meta.url))).toBe(true);
          }
        } else if (actionOwners.dataGraph.itemIds.includes(itemId) && rule.id === 'authored-data-direct-use') {
          const graphs = compileItemDataGraph(registry.items.get(itemId)!, 1);
          expect(graphs.some(graph => graph.eventType === rule.requiredTrigger), itemId).toBe(true);
          expect(metadataTriggers(itemId).size, `${itemId} must not gain a competing compiled owner`).toBe(0);
        } else expect([...metadataTriggers(itemId)], `${rule.id}: ${itemId}`).toContain(rule.requiredTrigger);
      }
    }
