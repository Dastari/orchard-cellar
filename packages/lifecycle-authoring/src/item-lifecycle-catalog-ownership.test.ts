...
describe('complete item lifecycle catalog ownership', () => {
  it('partitions every live item into one generated, data-graph, transaction or reviewed actionless owner', () => {
    ...
    expect([...callbackIds, ...inertIds, ...actionOwners.dataGraph.itemIds, ...actionOwners.furnitureTransactions.itemIds].sort()).toEqual(itemIds);
