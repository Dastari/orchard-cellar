...
describe('support administration caps', () => {
  it('resolves only safe integer authored overrides and keeps conservative defaults', () => {
    expect(resolveSupportCaps([
      { supportCap: 'itemsPerMutation', value: 7 },
      { supportCap: 'mutationsPerHour', value: -1 },
    ])).toEqual({ ...DEFAULT_SUPPORT_CAPS, itemsPerMutation: 7 });
  });

  it('resolves renamed active definitions semantically and fails closed for unavailable capabilities', () => {
    expect(resolveSupportCaps([
      { id: 'balance:renamed_item_limit', supportCap: 'itemsPerMutation', value: 9 },
      { id: 'balance:renamed_travel_limit', supportCap: 'teleportDistanceTiles', value: 12 },
    ])).toEqual({ ...DEFAULT_SUPPORT_CAPS, itemsPerMutation: 9, teleportDistanceTiles: 12 });
    expect(resolveSupportCaps([
      { supportCap: 'itemsPerMutation', value: 99, retired: true },
      { supportCap: 'walletDeltaBronzePerMutation', value: 900 },
      { supportCap: 'walletDeltaBronzePerMutation', value: 901 },
    ])).toEqual(DEFAULT_SUPPORT_CAPS);
  });
