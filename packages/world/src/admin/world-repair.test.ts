...
it('reports orphaned private stash custody without proposing destructive automatic repair',()=>{
  const report=planAdminValidateWorld(state({custody:[{
    entityKind:'hearth_stash_slot',entityId:'missing:0',holderIdentity:'missing',holderExists:false,parentExists:true,claims:1,
  }]}),'stash-custody',10n);
  expect(report.issues).toContainEqual(expect.objectContaining({code:'custody_holder_missing',repairable:false}));
});
