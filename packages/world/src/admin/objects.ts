...
function validStack(stack: AdminObjectStack): AdminObjectStack {
  const definition = itemDefinition(stack.itemKind);
  if (definition === null || !Number.isSafeInteger(stack.quantity)
    || stack.quantity <= 0 || stack.quantity > definition.maxStack
...
function applyMutation(
  ...
  request: Pick<AdminObjectPlanRequest, 'definitions' | 'blockedTiles' | 'resourceCandidates'>,
): AdminObjectState {
  switch (mutation.operation) {
    ...
    case 'set_container_slot': {
      ...
      slots[mutation.slot] = mutation.stack === null ? null : validStack(mutation.stack);
...
export function planAdminObjectMutation(
  ...
  const after = cloneState(applyMutation(before, request.mutation, request));
