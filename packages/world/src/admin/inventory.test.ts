...
function plan(
  input: AdminInventoryState,
  operation: AdminInventoryMutation,
...
describe('W2a administration inventory kernel', () => {
  ...
  it('requires an exact preview fingerprint and unchanged base version at commit', () => {
    ...
    const committed = planAdminInventoryMutation(input, {
      ...
      itemPolicy: bootstrapItemPolicy,
    });
    ...
    expect(() => planAdminInventoryMutation(input, {
      ...
      previewFingerprint: 'preview:wrong', nowMicros: 1_000_001n, itemPolicy: bootstrapItemPolicy,
    })).toThrowError(new AdminInventoryError('admin_preview_required'));
    expect(() => planAdminInventoryMutation(input, {
      mutation: commitMutation, expectedBaseVersion: 'inventory:stale',
      previewFingerprint: preview.previewFingerprint, nowMicros: 1_000_001n, itemPolicy: bootstrapItemPolicy,
  ...
  it('resolves arbitrary active item stack and quest-unique policy from the supplied registry', () => {
    const stackPolicy = customItemPolicy('admin_crystal', 7);
    const empty = state({
      containers: {
        ...state().containers,
        hotbar: container('hotbar', 2, [null, null]),
      },
    });
    const given = plan(empty, mutation({
      operation: 'give_items', targetIdentity: 'target', dryRun: true,
      stacks: [{ itemKind: 'admin_crystal', quantity: 8 }],
    }), stackPolicy);
    expect(given.after.containers.hotbar.slots).toEqual([
      { itemKind: 'admin_crystal', quantity: 7 },
      { itemKind: 'admin_crystal', quantity: 1 },
    ]);
    expect(() => plan(empty, mutation({
      operation: 'set_slot', targetIdentity: 'target', dryRun: true,
      slot: { area: 'hotbar', index: 0 }, stack: { itemKind: 'admin_crystal', quantity: 8 },
    }), stackPolicy)).toThrowError(new AdminInventoryError('admin_stack_invalid'));

    const uniquePolicy = customItemPolicy('admin_relic', 99, ['item.quest_unique']);
    const ownsRelic = state({
      containers: {
        ...state().containers,
        backpack: container('backpack', 2, [{ itemKind: 'admin_relic', quantity: 1 }, null]),
      },
    });
    expect(() => plan(ownsRelic, mutation({
      operation: 'give_items', targetIdentity: 'target', dryRun: true,
      stacks: [{ itemKind: 'admin_relic', quantity: 1 }],
    }), uniquePolicy)).toThrowError(new AdminInventoryError('admin_unique_item_conflict'));
  });
  ...
  it('fails closed for absent, retired, or omitted policy without rejecting preserved legacy rows', () => {
    ...
    for (const [itemKind, itemPolicy] of [
      ...
      expect(() => plan(input, mutation({
        operation: 'set_slot', targetIdentity: 'target', dryRun: true,
        slot: { area: 'hotbar', index: 1 }, stack: { itemKind, quantity: 1 },
      }), itemPolicy)).toThrowError(new AdminInventoryError('admin_stack_invalid'));
    }
    expect(() => planAdminInventoryMutation(input, {
      mutation: mutation({
        operation: 'set_slot', targetIdentity: 'target', dryRun: true,
        slot: { area: 'hotbar', index: 1 }, stack: { itemKind: 'apple', quantity: 1 },
      }),
      expectedBaseVersion: adminInventoryVersion(input),
      previewFingerprint: null,
      nowMicros: 1_000_000n,
    ...
    const withLegacyRow = state({
      containers: {
        ...state().containers,
        hotbar: container('hotbar', 2, [{ itemKind: 'removed_legacy_item', quantity: 400 }, null]),
      },
    });
    const activePolicy = customItemPolicy('admin_crystal', 7);
    const changed = plan(withLegacyRow, mutation({
      operation: 'set_slot', targetIdentity: 'target', dryRun: true,
      slot: { area: 'backpack', index: 0 }, stack: { itemKind: 'admin_crystal', quantity: 1 },
    }), activePolicy);
    expect(changed.after.containers.hotbar.slots[0]).toEqual({
