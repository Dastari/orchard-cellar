...
function uniqueCounts(state: AdminInventoryState, itemPolicy: ItemPolicyResolver): Map<string, number> {
  const counts = new Map<string, number>();
  for (const stack of allStacks(state)) if (itemPolicy.hasTag(stack.itemKind, UNIQUE_QUEST_ITEM_TAG)) {
...
function give(
  state: AdminInventoryState,
  mutation: AdminInventoryMutation & { readonly operation: 'give_items' },
  itemPolicy: ItemPolicyResolver,
): AdminInventoryState {
  let next = state;
  for (const requested of mutation.stacks) {
    const stack = validStack(requested, itemPolicy, false);
    let remaining = stack.quantity;
    for (const area of ['hotbar', 'backpack'] as const) {
      if (remaining === 0) break;
      const inserted = insertItemStackPartial(
        next.containers[area],
  ...
  requireUniquePolicy(state, next, itemPolicy);
  return next;
}
...
function remove(
  state: AdminInventoryState,
  mutation: AdminInventoryMutation & { readonly operation: 'remove_items' },
  itemPolicy: ItemPolicyResolver,
): AdminInventoryState {
  let next = state;
  for (const raw of mutation.stacks) {
    const requested = validStack(raw, itemPolicy, false);
    if (itemPolicy.hasTag(requested.itemKind, UNIQUE_QUEST_ITEM_TAG)) fail('admin_unique_item_conflict');
...
function setSlot(
  state: AdminInventoryState,
  slot: AdminSlotRef,
  requested: ItemStack | null,
  itemPolicy: ItemPolicyResolver,
): AdminInventoryState {
  const stack = requested === null ? null : validStack(requested, itemPolicy);
  ...
  if (slot.area === 'cursor') {
    ...
    if (!Number.isSafeInteger(slot.index) || slot.index < 0 || slot.index >= container.capacity
      || (stack !== null && !slotAcceptsItem(container, slot.index, stack.itemKind, itemPolicy))) {
      fail('admin_stack_invalid');
    }
...
function insertIntoCarried(
  state: AdminInventoryState,
  stack: ItemStack,
  itemPolicy: ItemPolicyResolver,
): { readonly state: AdminInventoryState; readonly remainder: number } {
  let next = state;
  let remaining = stack.quantity;
  for (const area of ['hotbar', 'backpack'] as const) {
    if (remaining === 0) break;
...
function clearCursor(state: AdminInventoryState, itemPolicy: ItemPolicyResolver): AdminInventoryState {
  if (state.cursor === null) return state;
  const inserted = insertIntoCarried(state, state.cursor, itemPolicy);
...
function drainOverflow(state: AdminInventoryState, itemPolicy: ItemPolicyResolver): AdminInventoryState {
  let next: AdminInventoryState = { ...state, overflow: [] };
  const remainder: ItemStack[] = [];
  for (const stack of state.overflow) {
    const inserted = insertIntoCarried(next, stack, itemPolicy);
    next = inserted.state;
...
function applyMutation(
  ...
  switch (mutation.operation) {
    case 'give_items': return give(state, mutation as AdminInventoryMutation & { readonly operation: 'give_items' }, itemPolicy);
    case 'remove_items': return remove(state, mutation as AdminInventoryMutation & { readonly operation: 'remove_items' }, itemPolicy);
    case 'set_slot': return setSlot(state, mutation.slot, mutation.stack, itemPolicy);
    case 'clear_cursor': return clearCursor(state, itemPolicy);
    case 'drain_overflow': return drainOverflow(state, itemPolicy);
  }
}
...
export function planAdminInventoryMutation(
  ...
  const after = cloneAdminInventoryState(applyMutation(
    before,
    request.mutation,
    request.itemPolicy ?? FAIL_CLOSED_ITEM_POLICY,
  ));
