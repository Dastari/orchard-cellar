...
describe('authored collision effects', () => {
  it('does not let state effects switch off the fixed native furniture base', () => {
    const content = registry();
    const definition = parseObjectDefinition({ id: 'object:furniture_rustic_chair', kind: 'object', schemaVersion: 1,
      displayName: 'Chair', components: {
        identity: { tags: ['furniture'] },
        placement: { item: 'item:furniture_rustic_chair', layer: 'object', spaces: ['residence'], facing: false,
          footprint: [[15]] },
        collision: { footprint: [[15]], blocksMovement: true }, furniture: [5, 6],
      } });
    const active = { ...content, objects: new Map([...content.objects, [definition.id, definition]]) };
    const row = { kind: 'furniture_rustic_chair', definitionId: definition.id, stateJson: '{}', open: false, lit: false };
    expect(planPlaceableCollisionEffect(active, row, true).stateJson).toBe('{}');
    expect(() => planPlaceableCollisionEffect(active, row, false)).toThrow('furniture_collision_fixed');
    const switchable = { ...definition, components: { ...definition.components,
      collision: { ...definition.components.collision!, when: { state: 'lit', equals: true } } } };
    const changed = { ...active, objects: new Map([...active.objects, [definition.id, switchable]]) };
    expect(() => planPlaceableCollisionEffect(changed, row, true)).toThrow('furniture_collision_fixed');
  });
...
describe('world object runtime compatibility', () => {
  it('preserves furniture attachments across light state updates without exposing them to authored effects', () => {
    const content = registry();
    const source = content.objects.get('object:furniture_townhouse_table_lamp')!;
    const definition = { ...source, components: { ...source.components, states: lamp.components.states!,
      light: lamp.components.light! } };
    const active = { ...content, objects: new Map([...content.objects, [definition.id, definition]]) };
    const row = { kind: 'furniture_townhouse_table_lamp', definitionId: definition.id, open: false, lit: false,
      stateJson: '{"lit":false,"intensity":2,"hearthFurnitureSupportId":"9007199254740993","hearthFurnitureRevision":"8"}' };
    expect(resolvePlaceableObject(active, row).stateJsonValid).toBe(true);
    const toggled = planPlaceableStateEffect(active, row, { toggleState: 'lit' });
    expect(JSON.parse(toggled.stateJson)).toEqual({ lit: true, intensity: 2, hearthFurnitureSupportId: '9007199254740993', hearthFurnitureRevision: '8' });
    expect(JSON.parse(planPlaceableLightEffect(active, row, true).stateJson))
      .toEqual({ lit: true, intensity: 2, hearthFurnitureSupportId: '9007199254740993', hearthFurnitureRevision: '8' });
    expect(() => planPlaceableStateEffect(active, row, { setState: { hearthFurnitureSupportId: '4' } })).toThrow();
    expect(() => planPlaceableStateEffect(active, { ...row, stateJson: '{"hearthFurnitureSupportId":"01"}' }, { toggleState: 'lit' }))
      .toThrow('furniture_state_invalid');
    expect(() => planPlaceableLightEffect(active, { ...row, stateJson: '{"hearthFurnitureSupportId":"01"}' }, true))
      .toThrow('furniture_state_invalid');
  });
