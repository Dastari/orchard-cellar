import {
  ...
  resolveObjectLight,
  HEARTH_FURNITURE_SUPPORT_STATE_KEY,
  HEARTH_FURNITURE_REVISION_STATE_KEY,
  hearthFurniturePersistentId,
  hearthFurnitureShapeForPlaceable,
  hearthFurniturePreservePlacementState,
...
function parseStateJson(
  ...
  try {
    ...
    for (const [name, value] of Object.entries(decoded)) {
      if (furniture && (name === HEARTH_FURNITURE_SUPPORT_STATE_KEY || name === HEARTH_FURNITURE_REVISION_STATE_KEY)) {
        if (!hearthFurniturePersistentId(value)) return { valid: false, state: {} };
        continue;
      }
...
export function resolvePlaceableObject(
  ...
  const parsed = parseStateJson(
    row.stateJson, declarations, hearthFurnitureShapeForPlaceable(registry, row) !== null,
  );
...
function stateMutation(
  ...
  const assign = (name: string, value: StateValue): void => {
    if (name === HEARTH_FURNITURE_SUPPORT_STATE_KEY || name === HEARTH_FURNITURE_REVISION_STATE_KEY) throw new Error(`behaviour_state_invalid:${name}`);
...
function planFromState(
  ...
  return Object.freeze({
    ...
    stateJson: hearthFurnitureShapeForPlaceable(registry, row) === null ? canonicalStateJson(state)
      : hearthFurniturePreservePlacementState(canonicalStateJson(state), row.stateJson ?? '{}'),
...
export function planPlaceableStateEffect(
  ...
  if (hearthFurnitureShapeForPlaceable(registry, row) !== null && !resolved.stateJsonValid) {
  ...
  return planFromState(registry, resolved, row, stateMutation(resolved, effect));
}
...
export function planPlaceableLightEffect(
  ...
  if (resolved.definition !== null && resolved.stateJsonValid && when !== undefined) {
    ...
    const planned = planFromState(registry, resolved, row, {
...
export function planPlaceableCollisionEffect(
  ...
  const furniture = hearthFurnitureShapeForPlaceable(registry, row);
  if (furniture !== null && (collision?.when !== undefined
    || enabled !== (furniture.layer === 'standing' && furniture.base !== undefined))) {
    throw new Error('furniture_collision_fixed');
  }
