...
/** Projects every authored static surface without depending on a space or
 * landmark definition id. Validation guarantees globally unique runtime ids. */
export function authoredSurfacePlans(
  spaces: Iterable<SpaceContentDefinition>,
  chunkTiles: number,
): readonly AuthoredSurfacePlan[] {
  return [...spaces].flatMap((space) => (space.surfaces ?? []).map((surface) => ({
