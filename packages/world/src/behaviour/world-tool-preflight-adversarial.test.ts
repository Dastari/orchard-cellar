...
describe('authored world-tool complete preflight', () => {
  ...
  it('keeps campfire and cellar validation prefixes read-only', () => {
    const campfire = between(
      worldSource,
      'function applyHarvestCampfireLifecycle(',
      'if (!mutate) {',
    );
    expect(campfire).toContain('activeCookingJob');
    expect(campfire).toContain('campfireWithinReach(');
    expectNoDurableMutation(campfire);
