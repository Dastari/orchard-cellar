...
describe('item authority integration', () => {
  ...
  it('uses the explicit placeable namespace for anvil repair', () => {
    ...
    expect(log).toEqual([
      ...
      'validate:consumeItem', 'validate:chargeBronze', 'validate:repairSelected',
      'validate:statistic', 'write:consumeItem', 'write:chargeBronze',
      'write:repairSelected', 'write:statistic',
    ]);
  });
