...
describe('authored woodcutting and mining tool world authority', () => {
  it('preflights one bounded worldTool effect before delegating to engine transactions', () => {
    ...
    expect(writer).toContain('applyHarvestPlaceableLifecycle(ctx, planned.targetId)');
    expect(writer).toContain('applyDigCellarTileLifecycle(ctx, planned.tileX, planned.tileY)');
  });

  it('preserves resource, authored placeable, and cellar transaction semantics verbatim', () => {
    ...
    ]) expect(placeable, marker).toContain(marker);
    expect(placeable).not.toContain("selected?.itemKind !== 'axe'");
    ...
    for (const marker of [
      ...
      'cellarOreKindAt(', 'itemKind: wallOutputKind!', "'rocks_broken'",
    ]) expect(cellar, marker).toContain(marker);
    expect(cellar).not.toContain("slot.itemKind !== 'pickaxe'");
    expect(cellar).not.toContain("itemKind: 'pebble'");
  });
