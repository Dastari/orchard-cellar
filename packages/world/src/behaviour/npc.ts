...
export interface AuthoredNpcProfilePlan {
  ...
  /** Empty means this dialogue NPC has no authored shop. Kept as a string for
   * durable compatibility with the existing world_merchant table schema. */
...
export function authoredNpcRowPlan(
  ...
  const initialFishing = definition.ai.kind === 'fishing_cycle'
    ? stepFishermanCycle(definition.ai, BigInt(definition.runtimeId), 'idle', authorityTick, authorityTick)
    : undefined;
  return {
...
export function authoredNpcProfilePlan(definition: NpcContentDefinition): AuthoredNpcProfilePlan | null {
  ...
  return {
    ...
    shopId: definition.shop?.slice('shop:'.length) ?? '',
  };
}

export interface AuthoredNpcWildlifeProfilePlan {
  readonly npcId: bigint;
  readonly species: string;
  readonly variant: number;
  readonly packId: bigint;
...
/** Projects fixed-wildlife metadata beside the retained NPC row. Position,
 * custody and home state come from that row rather than being reset from the
 * authored spawn point during reconnect or a content-head advance. */
export function authoredNpcWildlifeProfilePlan(
  definition: NpcContentDefinition,
  row: AuthoredNpcRow,
): AuthoredNpcWildlifeProfilePlan | null {
  const profile = definition.wildlifeProfile;
  if (profile === undefined || row.id !== BigInt(definition.runtimeId)) return null;
  return {
    npcId: row.id,
    species: profile.species,
    variant: profile.variant,
    packId: BigInt(profile.packId),
