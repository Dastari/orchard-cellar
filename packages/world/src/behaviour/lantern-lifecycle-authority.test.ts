...
describe('authored portable-light authority', () => {
  ...
  it('makes both light items reachable through the same off-hand lifecycle lane', () => {
    ...
    expect(clientSource).toContain('const definition = liveItemContentDefinition(snapshot, item.itemKind);');
    expect(clientSource).toContain('return definition?.light !== undefined');
    expect(clientSource).toContain("&& definition.equip?.slot === 'off_hand'");
