...
export function rejectingBehaviourEffectAdapters(
  ...
  return {
    ...
    restoreHunger: () => reject('restoreHunger'),
    repairSelected: () => reject('repairSelected'),
...
export function applyBehaviourEffects(
  ...
  for (let index = 0; index < effects.length; index += 1) {
    ...
    else if ('restoreHunger' in effect) writer.apply('restoreHunger', effect.restoreHunger, effect, index);
    else if ('repairSelected' in effect) writer.apply('repairSelected', effect.repairSelected, effect, index);
