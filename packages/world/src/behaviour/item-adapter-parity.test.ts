...
describe('item world-adapter row-write parity', () => {
  ...
  it('registers item and placeable handlers once in deterministic composition', () => {
    ...
    expect(bridge).toMatch(/registerPlaceableHandlers\(\s*registerLootHandlers\(/u);
  ...
  it('matches selected food, fruit buff, and orchard-tea durable writes', () => {
    expect(writer).toContain("effect.applyEffect.effectId === 'hunger'");
    expect(writer).toContain('runtimeFoodRestoreCenti(registry, selected.itemKind)');
    expect(writer).toContain('restoreHunger(survival, applied.stacks)');
    expect(writer).toContain("'food_eaten', 1n, authorityTick, selected.itemKind");
    ...
    expect(writer).toContain("applied.effectId === 'orchard_tea'");
    expect(writer).toContain("'orchard_tea_consumed', 1n, authorityTick");
  });
  ...
  it('matches atomic repair material, bronze, durability, and statistics writes', () => {
    expect(writer).toContain("effect.applyEffect.effectId === 'repair_selected'");
    ...
    expect(writer).toContain("'tools_repaired', 1n, authorityTick, selected.itemKind");
  ...
  it('keeps each concrete equipped light row and replicated light state in sync', () => {
    ...
    expect(writer).toContain('definition?.light === undefined');
    expect(writer).toContain("definition.equip?.slot !== 'off_hand'");
