...
describe('static-world readiness', () => {
  it('finds the whole-map dependencies the migration still has to remove', () => {
    ...
    // Step-4 baseline after S2b/S2c (17 -> 15): the collision dispatcher (2: the `off` branch calls
    // compiled directly, and the chunk source's compiled accessor, which the S2c audit shares),
    // combat policy (8, S3a -> 6) and document/resource consumers (5, S3b -> 1). Both dispatcher
    // sites stay until S3-final.
    expect(byId.get('server.whole-map-compile')!).toMatchObject({ count: 15, files: ['packages/world/src/index.ts'] });
