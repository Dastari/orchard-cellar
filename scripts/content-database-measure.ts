import { buildContentRegistry, contentDefinitionRowsHash, type ContentDefinitionRow } from '@orchard/sim';
...
async function subscribeContent(connection: DbConnection): Promise<void> {
  await timeout('content_subscription', new Promise<void>((resolve, reject) => {
    connection.subscriptionBuilder().onApplied(() => resolve())
      ...
      .subscribe([tables.contentHead.where((row) => row.packId.eq('live')), tables.runtimeContentDefinitions]);
  }));
}

function registryRows(connection: DbConnection): readonly ContentDefinitionRow[] {
  return [...connection.db.runtimeContentDefinitions.iter()].map((row) => ({ id: row.id, kind: row.kind, json: row.json }));
}
...
try {
  ...
  const initialDefinitionCount = [...publisher.db.runtimeContentDefinitions.iter()].length;
  const originalRows = registryRows(publisher);
  const head = publisher.db.contentHead.packId.find('live');
  if (head === null) throw new Error('content_measure_head_missing');
  if (initialInboundBytes.some((bytes) => bytes > INITIAL_SUBSCRIPTION_BUDGET_BYTES)) {
    throw new Error(`content_measure_initial_subscription_budget_exceeded:${JSON.stringify({
      definitionCount: initialDefinitionCount, budgetBytes: INITIAL_SUBSCRIPTION_BUDGET_BYTES,
      inboundBytes: summarizeSamples(initialInboundBytes),
    })}`);
  }
  ...
  observers.forEach((connection, index) => {
    connection.db.contentHead.onUpdate((_context, oldRow, newRow) => {
      ...
      queueMicrotask(() => {
        observerAtomicCounts[index] = [...connection.db.runtimeContentDefinitions.iter()].length;
  ...
  if (contentDefinitionRowsHash(rows) !== finalHead.contentHash) throw new Error('content_measure_publisher_hash_mismatch');
  for (const connection of observers) {
    const observerHead = connection.db.contentHead.packId.find('live');
    const observerRows = registryRows(connection);
    if (observerHead?.revision !== finalHead.revision || observerHead.contentHash !== finalHead.contentHash
      || observerRows.length !== expectedDefinitionCount || contentDefinitionRowsHash(observerRows) !== finalHead.contentHash) {
      throw new Error('content_measure_observer_payload_mismatch');
    }
    const byId = new Map(observerRows.map((row) => [row.id, row]));
    for (const original of originalRows) {
      const received = byId.get(original.id);
      if (received?.kind !== original.kind || received.json !== original.json) throw new Error('content_measure_original_payload_changed');
    }
  }
  ...
  console.log(JSON.stringify({
    ...
    payloadIntegrity: { verifiedObservers: observers.length, originalRowsPreservedPerObserver: originalRows.length, finalContentHash: finalHead.contentHash },
