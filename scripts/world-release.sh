...
Runs stage A of the guarded Tier-B transitional chest release. It rehearses both
stages on an isolated restore, but production stops at verified placeable reads
with all legacy rows retained for authenticated visual and two-client acceptance.
USAGE
  exit 64
}
...
module_source_manifest=''
printf 'World release stage A completed at placeable_reads; backup=%s pre-drain-expected=%s post-drain-expected=%s production-pre-drain=%s rehearsal-pre-log=%s rehearsal-post-log=%s production-pre-log=%s\n' \
  "$backup_directory" "$pre_drain_snapshot" "$post_drain_snapshot" \
