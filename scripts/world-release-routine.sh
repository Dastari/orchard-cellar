...
npm run build --workspace @orchard/studio -- --mode studio-production --outDir "$evidence/staged/packages/studio/dist"
...
traffic_stopped=false
printf 'deployed\n' > "$evidence/status"
printf 'Routine same-schema release completed; verified code rollback and 38-table parity evidence: %s\n' "$evidence"
