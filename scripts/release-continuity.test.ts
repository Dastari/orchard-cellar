...
describe('production continuity tooling', () => {
  ...
  it('closes both preview services before a build can replace their served artifacts', () => {
    ...
    const firstStudioBuild = release.indexOf('npm run build -w @orchard/studio');
    ...
    expect(stopStudio).toBeLessThan(firstStudioBuild);
  ...
  it('orders quiescence, checksum backup, isolated restore, no-delete publish, and parity verification', () => {
    ...
    expect(rehearsal).toContain('mktemp -d /tmp/orchard-world-restore.XXXXXX');
    ...
    const rehearsalCapture = rehearsal.indexOf('run world:rejoin-smoke -- capture "$pre_drain_snapshot"');
    const rehearsalVerify = rehearsal.indexOf('run world:rejoin-smoke -- verify "$pre_drain_snapshot"');
    ...
    expect(preBackupCredentialRefresh).toBeGreaterThan(release.indexOf('npm run build -w @orchard/studio -- --mode studio-production'));
  ...
  it('dry-runs finalization with fresh expectation paths and no publication surface', () => {
    ...
    try {
      ...
      chmodSync(token, 0o600);
      const result = spawnSync('bash', ['scripts/world-release-finalize.sh'], {
        cwd: new URL('..', import.meta.url), encoding: 'utf8',
  ...
  it('dry-runs post-drop retirement without reading tokens or touching services', () => {
    ...
    try {
      ...
      chmodSync(token, 0o600);
      const result = spawnSync('bash', ['scripts/world-release-retire-chests.sh'], {
        cwd: new URL('..', import.meta.url), encoding: 'utf8',
