...
it('prepares immutable URLs and a CAS request offline after verifying every blob',async()=>{
 ...
 try{
  ...
  const output=join(directory,'out');expect(await prepareChunkShadow(directory,output,'a'.repeat(64),0)).toMatchObject({chunks:1,published:false});
  expect(new Uint8Array(brotliDecompressSync(await readFile(join(output,'world','0',`${hash}.bin.br`))))).toEqual(blobs[0]);
  expect(JSON.parse(await readFile(join(output,'shadow-publication.json'),'utf8'))).toMatchObject({expectedRevision:0,contentHash:'a'.repeat(64)});
