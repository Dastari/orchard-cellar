...
migration_kind=${WORLD_RESTORE_MIGRATION_KIND:-legacy-chests}
...
# Production archives exceed the safe working set of this host's RAM-backed /tmp.
restore_parent=$repository/.release-work
install -d -m 0700 "$restore_parent"
[[ ! -L "$restore_parent" && "$(stat -f -c %T "$restore_parent")" != tmpfs ]] || {
  printf 'Restore workspace must be a disk-backed directory.\n' >&2; exit 65;
}
restore_root=$(mktemp -d "$restore_parent/orchard-world-restore.XXXXXX")
server_pid=''
...
cleanup() {
  ...
  [[ "$restore_root" = "$restore_parent"/orchard-world-restore.* ]] && rm -rf -- "$restore_root"
}
