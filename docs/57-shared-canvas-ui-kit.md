...
### Settled — Studio consistency, 2026-09-08

- All Studio routes, including UI Lab, use the same kit navigation rail and role-filtered route catalogue. The scrollbar has its own gutter beside full-width icons.
- Map drawers reserve intrinsic height for workspace controls and the publication footer; the placement palette consumes the remaining height and scrolls independently.
- Input centres are transparent. Single-line text is vertically centred; authored values keep their case and field captions use sentence case with ID/NPC/UI/JSON/XP acronyms.
- Buttons centre captions within their whole frame, reserving symmetric room for adornments. Disabled controls use the muted art family; enabled text and SVG symbols derive identical contrast from the actual button face.
- Pointer focus uses the control's authored outline. Generic keyboard focus uses the kit outline skin rather than a black rectangle; keyboard acceptance remains required.
- Tiled skins are assembled at native integer pixels in a bounded two-million-pixel cache before device scaling, avoiding tile seams at fractional DPR. Nine-slice art still tiles; it does not stretch.
- SVG symbols rasterize at the device-space size (with a bounded variant cache); sprite art retains nearest-neighbour rendering.
- Opening a retained popup must not rebuild its owner on pointer release. Browser acceptance includes clicking dropdowns, selecting options, dismissing them and keyboard navigation.
