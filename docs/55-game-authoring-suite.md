...
- Client content subscription: whole `content_definition` table; measure the initial
  payload (target ≤ 500 KB uncompressed) and gate on `content_head` before revealing
...
bootstrap registry now parses the committed 531 definitions in 21 JSON files through
